...
async function waitForControlPlaneDelivery(sessionId, previousTaskId, scenario) {
  try {
    ...
    console.error(`[stage1_control_plane_timeout_snapshot] ${snapshot}`);
...
function uiStateObserved(state, attrs, snapshot) {
  ...
  if (state === "blocked") {
    return (
      ...
      /blocked|waiting_permission/.test(
        snapshot?.task?.status ?? snapshot?.taskSession?.status ?? ""
      )
    );
  }
...
function mergeFinalDeliveryEvidence(left, right) {
  ...
  return {
    ...
    metadata: {
      ...leftMetadata,
      ...rightMetadata,
      blockers: uniqueRecords([
        ...(leftMetadata.blockers ?? []),
        ...(rightMetadata.blockers ?? []),
      ]),
