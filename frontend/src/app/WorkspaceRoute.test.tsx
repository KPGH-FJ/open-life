...
describe("Workspace route", () => {
  ...
  it("keeps approval disabled when the backend permission scope is incomplete", async () => {
    ...
    render(<ProductWorkbench workbenchDataSource={dataSource} initialSurface="workspace" />);
            ...
            data: {
              ...
              items: snapshot.tasksEnvelope.data!.items.map(task => ({
                ...
                pendingReviewItemRefs: [
                  { id: lifeModelItem.id, kind: "review_item" as const, label: "LifeModel change" },
                ],
              })),
            },
          },
  ...
  it("fails stale work state closed while preserving evidence access", async () => {
    const user = userEvent.setup();
    ...
    const dataSource = {
      ...
      load: async () => {
        ...
        return {
          ...
          tasksEnvelope: { ...snapshot.tasksEnvelope, status: "empty" as const },
        };
      },
    };
    ...
    const dataSource = {
      ...
      load: async () => {
        ...
        return {
          ...
          boundaryEnvelope: {
            ...snapshot.boundaryEnvelope,
            data: {
              ...snapshot.boundaryEnvelope.data!,
  ...
  it("opens Personal Intelligence from the canonical navigation", async () => {
    ...
    const dataSource = {
      ...fixture,
      loadPersonalIntelligence: async () => {
        const snapshot = await fixture.loadPersonalIntelligence();
        return {
          ...snapshot,
          lifeModelEnvelope: {
  ...
  it("keeps persistent approval separate from refreshed application", async () => {
    const fixture = workbenchFixtureDataSource("fixture-ready");
    const snapshot = await fixture.load();
  ...
  it("labels work actions by their exact operation and evidence boundary", async () => {
    const fixture = workbenchFixtureDataSource("fixture-ready");
    const snapshot = await fixture.load();
    const base = snapshot.reviewEnvelope.data!.items[0];
