...
import type { ReviewDispatchState } from "@/features/review/reviewDispatchContract";
import { scopedReviewItems, type WorkbenchSnapshot } from "@/app/workbenchDataSource";
...
export function reviewDecisionFeedback(
  ...
  switch (state.phase) {
    ...
    case "blocked":
      return {
        title: "当前不能记录决定",
        body: productErrorMessage(state.reason, "当前状态不允许记录这个决定。"),
        tone: "protection",
      };
    case "confirming":
      return null;
    ...
    case "failed":
      return {
        ...
        body: productErrorMessage(state.errorCode, "决定没有记录成功，请重试。"),
        tone: "error",
      };
    case "resolved":
      ...
      if (state.action.kind === "approve") {
        if (item.type === "external_write_action") {
          ...
          return evidence?.state === "confirmed" &&
            ...
            : {
                title: "文件结果尚未确认",
                body: "批准请求已经返回，但系统读模型没有提供匹配的确认收据；当前保持未完成或未知。",
                tone: "protection",
              };
        }
        if (item.decisionContext.actionContract) {
          return state.refreshed.materializationStatus === "applied"
            ...
            : {
                title: "动作结果尚未确认",
                body: "批准已返回，但刷新后的系统读模型尚未证明这一精确动作完成。",
                tone: "protection",
              };
        }
        return state.refreshed.materializationStatus === "applied"
          ...
          : {
              title: "已批准，尚未应用",
              body: "批准只记录决定；应用与完成仍需系统读模型提供独立结果。",
              tone: "neutral",
            };
      }
      return {
...
export function ReviewView({
  ...
  if (!snapshot || !envelope || envelope.status === "loading") {
    return (
      <div className="ol-work-page ol-work-page--centered" aria-busy="true">
  ...
  if (envelope.status === "error") {
    return (
      <div className="ol-work-page ol-work-page--centered">
  ...
  if (items.length === 0) {
    return (
      <div className="ol-work-page ol-work-page--centered">
        ...
        <div className="ol-work-inline-actions">
          <FoundationActionButton
            label={backLabel}
          ...
          <>
              ...
              <section className="ol-review-diff" aria-labelledby="review-change-title">
                ...
                <div className="ol-review-diff__columns">
                  <div>
                    <small>当前</small>
                    <strong>
                      {selectedItem.decisionContext.before?.summary ?? "系统未提供当前值"}
                    </strong>
                  </div>
                  <div>
                    <small>建议</small>
            ...
            {selectedItem.decisionContext.actionContract && (
              <details className="ol-review-progressive-detail">
                ...
                <section className="ol-review-rationale" aria-labelledby="action-contract-title">
                  <div className="ol-work-section-heading">
                    <span>允许范围</span>
            ...
            {learningContext && (
              ...
              >
                ...
                <dl>
                  ...
                  <div>
                    <dt>来源记录</dt>
                    <dd>{learningContext.sourceRefs.length} 条</dd>
                  </div>
                  <div>
                    <dt>冲突</dt>
                    <dd>
            ...
            <section className="ol-review-rationale" aria-labelledby="review-rationale-title">
              ...
              <dl>
                ...
                <div>
                  <dt>到期</dt>
                  <dd>{selectedItem.expiresAt ?? "系统未提供到期时间"}</dd>
                </div>
              </dl>
            </section>
                      ...
                      disabledReason={
                        disabledByEnvelope
                          ...
                          : disabledByBusy
                            ? "正在核对上一项决定。"
                            : unsupportedDispatch
                              ? action.disabledReason || "这项变更当前还不能应用。"
                              : action.enabled
                                ? undefined
                                : action.disabledReason || "系统未允许该决定。"
                      }
      ...
      >
        {permission && (
          <dl className="ol-permission-confirmation">
            ...
            <div>
              <dt>有效期</dt>
              <dd>{permission.expiresAt ?? "一次精确匹配或系统到期"}</dd>
            </div>
          </dl>
        )}
      </FoundationDialog>
