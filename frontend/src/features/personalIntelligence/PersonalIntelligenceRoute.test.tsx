...
import { ProductWorkbench } from "@/app/ProductWorkbench";
import { usePersonalIntelligenceController } from "./usePersonalIntelligenceController";

function renderPersonalIntelligence(fixtureId: Parameters<typeof workbenchFixtureDataSource>[0]) {
  const dataSource = workbenchFixtureDataSource(fixtureId);
  render(
    <ProductWorkbench
...
describe("Personal Intelligence route", () => {
  ...
  it("shows a structured canonical version instead of the legacy compatibility summary", async () => {
    ...
    vi.spyOn(dataSource, "loadPersonalIntelligence").mockResolvedValue(snapshot);
    render(
      <ProductWorkbench
        workbenchDataSource={dataSource}
        personalIntelligenceDataSource={dataSource}
    ...
    expect(screen.queryByText("查看 YAML 人类视图")).not.toBeInTheDocument();
    expect(screen.queryByText(/SQLite 中的版本化 JSON 才是权威/)).not.toBeInTheDocument();
    expect(screen.queryByLabelText("LifeModel YAML 人类视图")).not.toBeInTheDocument();
  ...
  it("clears a local LifeModel review banner after the backend reports a terminal decision", async () => {
    const dataSource = workbenchFixtureDataSource("fixture-ready");
    const pending = buildPersonalIntelligenceFixtureSnapshot("fixture-ready", "pending");
    const applied = buildPersonalIntelligenceFixtureSnapshot("fixture-ready", "applied");
    vi.spyOn(dataSource, "loadPersonalIntelligence")
      .mockResolvedValueOnce(pending)
      .mockResolvedValueOnce(applied);
    vi.spyOn(dataSource, "draftLifeModelChange").mockResolvedValue("proposal-focus-morning");
    const announce = vi.fn();
    const { result } = renderHook(() => usePersonalIntelligenceController(dataSource, announce));
  ...
  it("selects the exact newly created LifeModel review when older items remain", async () => {
    ...
    const refreshed = buildPersonalIntelligenceFixtureSnapshot("fixture-ready", "pending", [
      created,
    ]);
    vi.spyOn(dataSource, "loadPersonalIntelligence").mockResolvedValue(refreshed);
  ...
  it("keeps direct Memory controls available when the separate Review Center fails", async () => {
    const user = userEvent.setup();
    const dataSource = workbenchFixtureDataSource("fixture-ready");
    const snapshot = buildPersonalIntelligenceFixtureSnapshot("fixture-ready", "pending");
  ...
  it("keeps Memory review items out of LifeModel and opens them from Agent Memory", async () => {
    const user = userEvent.setup();
    const dataSource = workbenchFixtureDataSource("fixture-ready");
    const snapshot = buildPersonalIntelligenceFixtureSnapshot("fixture-ready", "pending");
    const memoryItem = {
    ...
    vi.spyOn(dataSource, "loadPersonalIntelligence").mockResolvedValue(snapshot);
    const loadWorkbench = dataSource.load.bind(dataSource);
    vi.spyOn(dataSource, "load").mockImplementation(async conversationId => {
      const workbenchSnapshot = await loadWorkbench(conversationId);
      if (workbenchSnapshot.reviewEnvelope.data) {
        workbenchSnapshot.reviewEnvelope.data = {
          ...workbenchSnapshot.reviewEnvelope.data,
          items: [memoryItem, ...workbenchSnapshot.reviewEnvelope.data.items],
        };
      }
    ...
    await user.click(await screen.findByRole("tab", { name: /Agent 记忆.*工作连续性/ }));
    const memorySuggestions = screen.getByRole("region", { name: "待处理的记忆建议" });
    expect(memorySuggestions).toBeVisible();
    expect(
      within(memorySuggestions).getByText(memoryItem.decisionContext.after.summary)
    ).toBeVisible();
    await user.click(within(memorySuggestions).getByRole("button", { name: "查看并决定" }));
    expect(await screen.findByRole("heading", { name: "Add a memory", level: 2 })).toBeVisible();
  });

  it("applies a direct reversible Memory correction without changing LifeModel", async () => {
    const user = userEvent.setup();
    const dataSource = renderPersonalIntelligence("fixture-ready");
    const correctMemory = vi.spyOn(dataSource, "correctMemory");

    await user.click(await screen.findByRole("tab", { name: /Agent 记忆.*工作连续性/ }));
    await user.click(screen.getByRole("button", { name: "编辑" }));
    const editor = screen.getByLabelText("编辑记忆");
    ...
    expect(screen.getByText("Memory 已纠正；此前内容保留为可回滚历史。")).toBeInTheDocument();
  });
  ...
  it("opens the exact review without deciding and returns approved-not-applied after refresh", async () => {
    ...
    expect(screen.getByText("这项建议已经批准，但当前还不能应用。")).toBeInTheDocument();
  ...
  it("renders verified applied only for the exact refreshed proof", async () => {
    const { container } = render(
      (() => {
        const dataSource = workbenchFixtureDataSource("fixture-personal-intelligence-applied");
        return (
          <ProductWorkbench
            workbenchDataSource={dataSource}
            personalIntelligenceDataSource={dataSource}
    ...
    expect(container.querySelector('[data-intelligence-lifecycle="applied"]')).toBeInTheDocument();
    expect(container.querySelector(".ol-status-label--success")).toBeInTheDocument();
  });

  it("fails stale Personal Intelligence state closed and keeps decision actions out of the page", async () => {
    const { container } = render(
      (() => {
        const dataSource = workbenchFixtureDataSource("fixture-stale");
        return (
          <ProductWorkbench
            workbenchDataSource={dataSource}
            personalIntelligenceDataSource={dataSource}
  ...
  it("builds the first v2 item from an explicitly selected long-term answer", async () => {
    const user = userEvent.setup();
    const dataSource = renderPersonalIntelligence("fixture-empty");
