...
export type PersonalIntelligenceLifecyclePresentation = {
...
export function isPersonalIntelligenceReviewItem(item: ReviewItem): boolean {
  return durableTypes.has(item.type);
}

export function personalIntelligenceReviewItems(
  snapshot: PersonalIntelligenceSnapshot | null
): ReviewItem[] {
  if (!snapshot || !["ready", "stale"].includes(snapshot.reviewEnvelope.status)) return [];
  return (snapshot.reviewEnvelope.data?.items ?? []).filter(isPersonalIntelligenceReviewItem);
}

function requiredOwnerReady(snapshot: PersonalIntelligenceSnapshot, item: ReviewItem): boolean {
...
function hasExactLifeModelAppliedProof(
...
export function personalIntelligenceLifecyclePresentation(
  snapshot: PersonalIntelligenceSnapshot | null,
  ...
  if (ownerStatuses.some(status => status === "error")) {
    return {
      ...
      label: "状态不可用",
      status: "error",
      detail: "至少一个系统读模型读取失败，当前不能确认长期状态。",
    };
  }
  ...
  if (item.materializationStatus === "applying") {
    return {
      ...
      label: "正在应用",
      status: "waiting",
      detail: "系统确认应用过程已经开始，但尚未形成最终长期状态。",
    };
  }
  ...
  if (item.materializationStatus === "rolled_back") {
    return {
      ...
      label: "已回滚",
      status: "waiting",
      detail: "系统读模型确认此前应用已回滚；当前没有可调用的再次应用动作。",
    };
  }
...
export function personalIntelligenceContext(
  snapshot: PersonalIntelligenceSnapshot | null,
  item: ReviewItem | null
): WorkbenchContextSummary {
  const state = personalIntelligenceLifecyclePresentation(snapshot, item);
  return {
    eyebrow: "个人智能",
    title: "关于我与 Agent 记忆",
...
export function personalIntelligenceInspector(
  snapshot: PersonalIntelligenceSnapshot | null,
  item: ReviewItem | null,
    ...
    nextAction:
      state.lifecycle === "pending_review" || state.lifecycle === "deferred"
        ...
        : state.lifecycle === "approved_not_applied"
          ? "等待系统提供应用结果；当前没有可执行动作时保持只读。"
          : state.lifecycle === "failed"
            ? "查看失败证据；等待系统提供可验证的重试或回滚动作。"
