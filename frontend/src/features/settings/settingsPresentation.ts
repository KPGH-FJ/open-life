...
export function searchUsesSelectedProviderCredential(config: AppConfig): boolean {
  if (config.prefer_local_model) {
    return false;
  }
  const configuredSearch = (config.system?.search_provider ?? "auto").trim().toLowerCase();
  const selectedProvider = (config.llm.provider ?? "").trim().toLowerCase();
  const effectiveSearch = configuredSearch === "auto" ? selectedProvider : configuredSearch;
  if (
    effectiveSearch !== selectedProvider ||
    !["deepseek", "openrouter"].includes(selectedProvider)
  ) {
    return false;
  }
  try {
    const url = new URL(config.llm.openai_base.trim());
    if (
      url.protocol !== "https:" ||
...
export function effectiveSearchCredentialState(
  config: AppConfig
): "stored" | "entered" | "missing" {
  return searchUsesSelectedProviderCredential(config)
    ? credentialState(config)
    : searchCredentialState(config);
}
        ...
        ? {
            system: {
              ...
              searchCredentialPresence:
                effectiveSearchCredentialState(config) === "missing" ? "missing" : "present",
            },
          }
        : {}),
    };
...
export function validateSettingsDraft(config: AppConfig | null): SettingsDraftValidation {
  ...
  if (
    ...
    effectiveSearchCredentialState(config) === "missing"
  ) {
    return {
      canSave: false,
...
export function connectionTestPresentation(
  ...
  if (result.validation_status === "runtime_generation_incoherent") {
    return {
      ...
      detail: "网络请求没有继续；需要先恢复系统配置与执行代一致性。",
      status: "error",
    };
  }
