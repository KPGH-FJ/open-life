    ...
    return (
      <div className={`ol-settings-page${safeModeActive ? "" : " ol-settings-page--centered"}`}>
        ...
        <FoundationNotice title="设置暂不可用" tone="error">
          <p>系统没有返回可编辑配置。页面不会用默认 Provider、地址或隐私结论代替。</p>
        </FoundationNotice>
        <FoundationActionButton
          label="重新读取"
        ...
        <fieldset className="ol-settings-form" disabled={busy}>
          ...
          <section className="ol-settings-section" aria-labelledby="ol-settings-local-title">
            ...
            <FoundationTextField
              ...
              label="本地模型名称"
              description="使用系统清理配置中的模型标识，不在页面探测已安装模型。"
          ...
          <section className="ol-settings-section" aria-labelledby="ol-settings-search-title">
            ...
            <label className="ol-settings-select-field" htmlFor="ol-settings-search-provider">
              ...
              <span className="ol-settings-select-field__description">
                默认复用当前模型路由支持的网页搜索能力；也可明确选择独立搜索服务。
              </span>
              <select
            ...
            {searchProvider === "auto" && searchReusesModelCredential && (
              <FoundationNotice title="自动使用当前模型路由">
                <p>网页搜索复用当前 Provider、模型和凭据，不需要再次配置或逐次确认。</p>
              </FoundationNotice>
            )}
            {searchProvider === "auto" && !searchReusesModelCredential && (
              <FoundationNotice title="当前模型路由没有可用的托管搜索">
                <p>请选择 Brave、SearXNG 或其他已配置搜索服务；OpenLife 不会静默切换 Provider。</p>
              </FoundationNotice>
            )}
            ...
            {searchProvider === "deepseek" && searchReusesModelCredential && (
              <FoundationNotice title="复用当前 DeepSeek 凭据">
                <p>模型与网页搜索都连接 DeepSeek 官方服务，因此不会重复保存或再次询问凭据。</p>
              </FoundationNotice>
            )}
        ...
        <fieldset className="ol-settings-form" disabled={busy}>
          ...
          <section className="ol-settings-section" aria-labelledby="ol-settings-network-title">
            ...
            <label className="ol-settings-select-field" htmlFor="ol-settings-network-default">
              ...
              >
                {networkDefault === undefined && <option value="unknown">系统未返回</option>}
...
function ProductDiagnosticsPanel({ controller }: { controller: SettingsController }) {
    ...
    return (
      ...
      >
        <FoundationNotice title="产品诊断不可用" tone="error">
          <p>系统没有返回可用的产品诊断；页面不会从日志或旧状态推断健康状态。</p>
        </FoundationNotice>
        <FoundationActionButton
          label="重新读取"
          icon={<RefreshCw size={18} strokeWidth={1.75} aria-hidden="true" />}
        ...
        <dl className="ol-settings-fact-list">
          {diagnostics.stores.map(store => (
            <div key={store.store}>
              ...
              <dd>
                {store.status}
                {store.reasonCode ? ` · ${productErrorMessage(store.reasonCode)}` : ""}
              </dd>
            </div>
          ))}
        </dl>
        ...
        >
          <ul className="ol-settings-diagnostic-list">
            {diagnostics.blockerCodes.map(code => (
              <li key={code}>{productErrorMessage(code)}</li>
            ))}
          </ul>
        </FoundationNotice>
      )}
...
function SafeModeNotice({ controller }: { controller: SettingsController }) {
...
function CredentialInitializationPanel({ controller }: { controller: SettingsController }) {
        ...
        <FoundationNotice title="初始化未完成" tone="error" live>
          <p>
            {report?.blockedReason ??
              "系统没有证明全部初始化步骤和补偿步骤完成；当前继续保持阻塞。"}
          </p>
        </FoundationNotice>
      ) : phase === "failed" ? (
        <FoundationNotice title="初始化已取消或失败" tone="protection" live>
          <p>没有获得成功证明；当前进程和系统快照都不会被标记为可用。</p>
        </FoundationNotice>
      ) : (
        <p>
          {accessRecovery
