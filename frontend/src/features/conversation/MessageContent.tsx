...
import { openExternalHttpsSource } from "@/ipc/work";
...
function unescapeSourceLabel(value: string): string {
...
function parseWebSource(line: string): WorkspaceMessageSource | null {
  ...
  return {
    id: match[1],
    label: unescapeSourceLabel(match[2]),
    detail: unescapeSourceLabel(match[4]),
    kind: "web",
    url,
  };
}
...
function parseResourceSource(line: string): WorkspaceMessageSource | null {
  ...
  return {
    id: match[1],
    label: unescapeSourceLabel(match[2]),
    detail: unescapeSourceLabel(match[3]),
    kind: "resource",
  };
}
