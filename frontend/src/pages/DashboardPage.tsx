...
function MiniLineChart({
  ...
  return (
    <svg width={width} height={height} className="overflow-visible">
      ...
      <text x={pad} y={height - 2} fontSize="10" fill="#9ca3af">
        {new Date(data[0].recorded_at).toLocaleDateString("zh-CN", {
          month: "numeric",
          day: "numeric",
        })}
      </text>
      <text x={width - pad} y={height - 2} textAnchor="end" fontSize="10" fill="#9ca3af">
        {new Date(data[data.length - 1].recorded_at).toLocaleDateString("zh-CN", {
          month: "numeric",
          day: "numeric",
        })}
      </text>
...
function summarizeStateDimension(
  ...
  if (dimension.min_threshold !== undefined && latest < dimension.min_threshold)
    thresholdStatus = "low";
  if (dimension.max_threshold !== undefined && latest > dimension.max_threshold)
...
export default function DashboardPage() {
  ...
  const warningText = (label: string, error: unknown) =>
    `${label}加载失败：${error instanceof Error ? error.message : String(error)}`;
  ...
  const onRecordState = async () => {
    ...
    const alertDays = stateInputAlertDays.trim()
      ? parseInt(stateInputAlertDays.trim(), 10)
      : undefined;
    await recordState(
      name,
      val,
      unit,
      stateInputNote.trim() || undefined,
      minVal,
      maxVal,
      alertDays
    );
  ...
  const lowestDimensionValue = lowestDimension
    ...
    <div className="h-full overflow-auto bg-[#f4efe7] p-4 sm:p-6">
      <div className="max-w-6xl mx-auto space-y-5">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {(nextActions.length > 0
                  ...
                  : [
                      {
                        title: "开始一次今日规划",
                        detail: "进入对话，让 OpenLife 帮你把今天切成可执行步骤。",
                        to: "/chat",
                      },
                    ]
                ).map((action, index) => (
              ...
              <div className="rounded-2xl border border-white/70 bg-white/70 p-4">
                ...
                <div className="mt-2 h-2 rounded-full bg-stone-200">
                  <div
                    ...
                    style={{ width: `${Math.max(0, Math.min(100, overallCompletion))}%` }}
                  />
                </div>
              </div>
              <div className="rounded-2xl border border-white/70 bg-white/70 p-4">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-1 text-xs text-gray-500">
                  按这个顺序推进，就能完成“建模 → 对话 → 查看 → 校准 → 回滚”的核心路径。
                </div>
              </div>
              <button
                onClick={refreshAll}
                className="rounded-md border bg-white px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50"
              >
                刷新状态
              </button>
            </div>
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <div className="mt-1 text-xs leading-5 text-amber-800">
                  OpenLife 已经分析了你的人生模型并生成了更新建议。在 Review Center
                  确认前，这些建议不会写入模型。
                </div>
              </div>
              <Link
          ...
          <div className="bg-white border rounded-xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mt-1">
                你的仪表盘还是空白的。花 10 分钟通过「构建」模式建立人生模型，OpenLife
                就能为你提供真正个性化的对话与洞察。
              </p>
            </div>
            <Link
              to="/builder"
              className="inline-flex items-center justify-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 shrink-0"
            >
              ...
              {data.dailyGoals.map((g, i) => (
                <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2">
                  <button
                    ...
                    className={`w-5 h-5 rounded border flex items-center justify-center ${g.done ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-300"}`}
                  >
                  ...
                  {editingGoalIndex === i ? (
                    <>
                      <input
                        ...
                        onKeyDown={e => e.key === "Enter" && onSaveEditGoal(i)}
                        className="flex-1 border rounded px-2 py-1 text-sm"
                        autoFocus
                      />
                      <button onClick={() => onSaveEditGoal(i)} className="text-green-600">
                        <Check size={16} />
                      </button>
                      <button onClick={() => setEditingGoalIndex(null)} className="text-gray-500">
                        <X size={16} />
                      </button>
                    </>
                  ) : (
                    <>
                      <span
                        className={`flex-1 text-sm ${g.done ? "line-through text-gray-400" : "text-gray-800"}`}
                      >
                        {g.name}
                      </span>
                      <button
                        onClick={() => startEditGoal(i, g.name)}
              ...
              {addingGoal ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={onAddGoal} className="text-green-600">
                    <Check size={18} />
                  </button>
                  <button onClick={() => setAddingGoal(false)} className="text-gray-500">
                    <X size={18} />
                  </button>
                </div>
              ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="bg-white border rounded-xl p-5">
            <div className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <Zap size={18} className="text-indigo-600" /> 能力雷达
            </div>
          ...
          <div className="bg-white border rounded-xl p-5 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold text-gray-800 flex items-center gap-2">
                <Activity size={18} className="text-indigo-600" /> 状态趋势
              </div>
              <button
                onClick={() => setShowStateModal(true)}
                className="inline-flex items-center gap-1 text-xs bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700"
              >
                <Plus size={12} /> 记录
              </button>
            </div>
            <div className="flex-1 overflow-auto space-y-2">
              ...
              {data.dimensions.slice(0, 5).map(dim => (
                <div
                  ...
                  className={`cursor-pointer border rounded-lg p-3 hover:shadow-sm transition ${selectedDimension === dim.name ? "border-indigo-500 ring-1 ring-indigo-500" : ""}`}
                >
                  <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-gray-400">
                    {dim.min_threshold !== undefined || dim.max_threshold !== undefined
                      ? `阈值 ${dim.min_threshold ?? "-"} ~ ${dim.max_threshold ?? "-"}`
                      : "未设置阈值"}
                  </div>
                </div>
              ))}
              ...
              <div className="mt-3 border-t pt-3">
                ...
                {trendSummary && (
                  <div className="mt-3 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div
                        className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium ${trendBadge(trendSummary.direction)}`}
                      >
                      ...
                      <div className="text-xs text-gray-500">
                        {trendSummary.previous === null
                          ? "首条记录"
                          : `较上一条 ${trendSummary.delta >= 0 ? "+" : ""}${trendSummary.delta.toFixed(1)} ${selectedDimensionModel?.unit ?? ""}`}
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <div className="rounded-lg bg-indigo-50 px-3 py-2">
                        ...
                        <div className="text-sm font-semibold text-indigo-900">
                          {trendSummary.latest.toFixed(1)} {selectedDimensionModel?.unit}
                        </div>
                      </div>
                      <div className="rounded-lg bg-slate-50 px-3 py-2">
                        <div className="text-[10px] text-slate-500">平均</div>
                        <div className="text-sm font-semibold text-slate-900">
                          {trendSummary.average.toFixed(1)} {selectedDimensionModel?.unit}
                        </div>
                      </div>
                      <div className="rounded-lg bg-amber-50 px-3 py-2">
                        <div className="text-[10px] text-amber-500">波动</div>
                        <div className="text-sm font-semibold text-amber-900">
                          {trendSummary.min.toFixed(1)} - {trendSummary.max.toFixed(1)}
                        </div>
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          <div className="bg-white border rounded-xl p-4">
            ...
            <div className="text-[10px] text-gray-400">
              {completedGoals} / {allGoals.length}
            </div>
          </div>
          <div className="bg-white border rounded-xl p-4">
          ...
          <div className="bg-white border rounded-xl p-4">
            ...
            <button
              ...
              className="text-xs bg-gray-900 text-white px-2 py-1 rounded hover:bg-gray-800 disabled:opacity-50"
            >
              {calibrationLoading ? "生成中" : "报告"}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 bg-white border rounded-xl p-5">
            <div className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <Brain size={18} className="text-indigo-600" /> 语义记忆检索
            </div>
              ...
              {memories.map((m, i) => (
                <div key={i} className="bg-gray-50 rounded-lg p-3 text-sm">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    source: {m.chunk.source} · 相关度: {m.score.toFixed(3)}
                  </div>
                </div>
              ))}
              {memories.length === 0 && (
          ...
          <div className="bg-white border rounded-xl p-5">
            <div className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <RefreshCw size={18} className="text-indigo-600" /> 微进化
            </div>
            <div className="flex flex-col gap-3">
              <button
                onClick={handleRunMicroEvolution}
                className="inline-flex items-center justify-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700"
              >
                <RefreshCw size={16} /> 运行微进化
              </button>
              {evolutionMsg && (
              ...
              {calibration && (
                <div className="space-y-2 text-sm">
                  <div className="text-xs text-gray-500">
                    {calibration.summary_text.slice(0, 60)}...
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1 bg-gray-50 rounded p-2 text-center">
                      <div className="text-xs text-gray-400">正向</div>
                      <div className="font-semibold">{calibration.feedback_up}</div>
                    </div>
                    <div className="flex-1 bg-gray-50 rounded p-2 text-center">
                      <div className="text-xs text-gray-400">负向</div>
                      <div className="font-semibold">{calibration.feedback_down}</div>
                    </div>
                  </div>
                </div>
              )}
                ...
                data.gaps.slice(0, 3).map((gap, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full ${gap.severity === "high" ? "bg-rose-100 text-rose-700" : gap.severity === "medium" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}`}
                      >
                        {gap.severity}
                      </span>
                    </div>
                    <div className="mt-1 text-sm text-gray-700">
                      关键能力：{gap.skill_name} · 当前 {gap.current_level}/10 · 建议目标{" "}
                      {gap.target_level}/10
                    </div>
                ...
                data.alignments.slice(0, 3).map((issue, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full ${issue.severity === "high" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}
                      >
                        {issue.severity}
                      </span>
                    </div>
                    ...
                    {issue.related_values.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {issue.related_values.map(value => (
                          <span
                            key={value}
                            className="rounded-full bg-white px-2 py-0.5 text-[10px] text-rose-700 border border-rose-100"
                          >
                            {value}
                          </span>
                        ))}
