  ...
  return (
    <div className="h-full overflow-auto p-6">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow p-6 space-y-6">
            ...
            <section className="space-y-4 border-t pt-4">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 space-y-2">
                <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  ModelRouter 仍为灰度能力；AgentRun trace 会记录 privacy、retry、fallback 与
                  provider health 是否为估算值。
                </div>
                <div className="flex items-center justify-between">
                  <span>当前后端</span>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span>
                    {routerStatus
                      ? `${Math.round(routerStatus.latency_threshold_us / 1000)}ms`
                      : "-"}
                  </span>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white">
                ...
                <div className="divide-y divide-slate-100">
                  {(modelRouterStatus?.providers ?? []).map(provider => (
                    <div
                      key={provider.name}
                      className="grid gap-2 px-4 py-3 text-sm md:grid-cols-5"
                    >
                        ...
                        <button
                          onClick={() =>
                            setConfig(prev => ({
                              ...
                              system: {
                                ...prev.system,
                                safe_paths: (prev.system?.safe_paths ?? []).filter(
                                  (_, i) => i !== idx
                                ),
                              },
                            }))
        ...
        {/* Privacy & Security Tab (continued) */}
        {activeTab === "privacy" && (
          <>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setEvolutionResult(
                        `已应用规则 ${res.applied_rules.length} 条\n${res.summary}`
                      );
                    } catch (e: any) {
