...
import { useGovernedActionJourney } from "./useGovernedActionJourney";

describe("useGovernedActionJourney", () => {
  it("refreshes dependent conversation state after a retry changes the canonical task", async () => {
    ...
    const { result } = renderHook(() =>
      useGovernedActionJourney(dataSource, announce, refreshDependentState)
    );
  ...
  it("re-reads task and conversation truth when retry reports an error after durable mutation", async () => {
    const fixture = workbenchJourneyFixtureDataSource("fixture-ready");
    const initial = await fixture.load();
    const taskId = initial.tasksEnvelope.data!.items[0].canonicalTaskId;
    const control: TaskControl = {
