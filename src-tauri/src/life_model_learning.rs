...
use openlife_core::agent::{
    LifeModelLearningCapture, LifeModelLearningCaptureReceipt, LifeModelLearningExplicitness,
    LifeModelLearningSensitivity, MainChatMemoryCandidate, MemoryCandidateKind, MemoryDestination,
};
...
fn typed_learning_candidate(candidate: &MainChatMemoryCandidate) -> Option<TypedLearningCandidate> {
    ...
    let explicitly_long_term = lower.contains("long-term")
        ...
        || claim.contains("长期")
        || claim.contains("以后");
    explicitly_long_term.then_some(TypedLearningCandidate {
