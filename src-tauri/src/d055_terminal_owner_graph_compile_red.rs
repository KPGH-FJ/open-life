...
use crate::terminal_owner_write_gateway::TerminalOwnerWriteGateway;
...
async fn d055_target_file_backed_successor_uses_verified_owner_local_receipt() {
    tokio::time::timeout(SCENARIO_TIMEOUT, async {
        ...
        let epoch = event_store
            .open_terminal_owner_epoch_from_canonical_user_message(
                &operation_id,
                &operation_id,
                canonical_message,
            )
            .expect("open one epoch by consuming an opaque canonical-message commit");
        assert_eq!(
...
async fn d055_target_cross_store_owner_commit_reconciles_successor_exactly_once_after_restart() {
    tokio::time::timeout(SCENARIO_TIMEOUT, async {
        ...
        let epoch = event_store
            ...
            .expect("open message-bound terminal epoch");
        assert_eq!(
            epoch.canonical_user_message_ref(),
            canonical_receipt.canonical_ref
        );
        ...
        gateway
            .install_fail_after_owner_local_commit_before_successor_confirm_for_test(&proposal_id)
        ...
        assert_eq!(
            error.to_string(),
            "injected_failure_after_owner_local_commit_before_successor_confirm"
        );
        ...
        // An unqueryable external effect is a separate durable counterexample:
        // reconciliation may observe it, but must never dispatch it again.
        let unknown_external = external_unknown_proposal();
        let unknown_external_id = unknown_external.id.clone();
        proposal_store
            .create_proposal(&unknown_external)
            .expect("stage independent external-write counterexample");
        let unknown_claim = proposal_store
            .claim_dispatch(&unknown_external_id)
            .expect("claim external counterexample")
            .expect("one external claimant");
        assert!(proposal_store
            .mark_dispatch_unknown(
                &unknown_external_id,
                &unknown_claim,
                "remote_effect_unqueryable",
            )
            .expect("persist external unknown truth"));
        ...
        assert_eq!(
            reopened_proposals
                .dispatch_state(&unknown_external_id)
                .expect("read external unknown after reconciliation")
                .as_deref(),
            Some("unknown")
        );
        assert_eq!(
            reopened_proposals
                .get_proposal(&unknown_external_id)
                .expect("read external unknown Proposal")
                .expect("external unknown Proposal exists")
                .status,
            ProposalStatus::Pending
        );
        ...
        assert!(events.iter().all(|event| {
            event.payload["causeRef"].as_str() != Some(unknown_external_id.as_str())
        }));
    })
    .await
...
async fn d055_target_final_insert_and_sealed_cas_are_one_event_store_transaction() {
    tokio::time::timeout(SCENARIO_TIMEOUT, async {
        ...
        let epoch = event_store
            ...
            .expect("open epoch from opaque canonical-message authority");
        assert_eq!(
            epoch.canonical_user_message_ref(),
            canonical_receipt.canonical_ref
        );
