...
#[cfg(feature = "dev-extensions")]
use crate::storage::mcp_audit_keyring_path;
use crate::storage::{
    app_data_dir, load_mcp_audit_keyring_from_path, mcp_audit_keyring_bytes, privacy_policy_path,
    save_mcp_audit_keyring_to_path, save_privacy_policy_to_path, McpAuditKeyringBytes,
    McpAuditKeyringLoad,
};
use crate::AppState;
...
fn danger_action_requires_native_confirmation(action_type: &str) -> bool {
    matches!(
        action_type,
        "mcp_audit_export" | "mcp_audit_cleanup" | "mcp_audit_key_rotation" | "vector_rebuild"
    )
}
...
pub async fn get_danger_action_preflight(
    ...
    let effective_affected_count = if action_type == "vector_rebuild" && affected_count.is_none() {
        ...
        Some(store.message_count().map_err(AppError::from)?)
    } else {
        affected_count
    };
...
mod tests {
    ...
    fn danger_action_preflight_safe_mode_blocks_destructive_actions() {
        ...
        for action_type in ["mcp_audit_export"] {
            let view = danger_action_preflight_for_action(action_type, true).unwrap();
            assert!(!view.writes_durable_state);
            assert!(!view.safe_mode_blocked);
            assert!(view.final_action_enabled);
