...
use crate::agent::{
    LifeDomain, LifeEventDraft, LifeEventPrivacyLevel, LifeEventSourceRef, LifeEventSourceType,
    LifeEventSourceVerification, LifeEventStore, RiskLevel,
};
...
fn agent_run_revision_and_stale_permit_resist_a_b_a_updates() {
    ...
    let execution = fixture
        ...
        .unwrap();
    let (stale, draft) = crate::agent::life_event_store::issue_life_event_create_permit(
        fixture.commit.proof(),
...
fn deterministic_life_event_policy_rejects_medium_and_high_risk_or_sensitivity() {
    use crate::agent::life_event_store::LifeEventSensitivity;
    ...
    for (risk, sensitivity, expected) in [
        ...
        let error = crate::agent::life_event_store::issue_life_event_create_permit(
            fixture.commit.proof(),
            policy,
            &execution,
            &uuid::Uuid::new_v4().to_string(),
...
fn stale_permit_stays_invalid_and_physical_recreate_cannot_bypass_tombstone() {
    ...
    let (stale_after_restore, draft_after_restore) =
        crate::agent::life_event_store::issue_life_event_create_permit(
            fixture.commit.proof(),
            fixture.policy_proof(),
            &execution,
            &uuid::Uuid::new_v4().to_string(),
