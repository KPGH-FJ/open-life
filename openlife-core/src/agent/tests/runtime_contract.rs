use crate::agent::{
    ...
    AgentTaskKind, EvidenceQuery, EvidenceStore, LifeEventDraft, RuntimeInput, RuntimeOutput,
...
#[test]
fn runtime_input_carries_task_memory_tools_typed_policy_and_budget() {
    ...
    assert_eq!(
        input.policy_context.provider_authorization().data_route(),
...
#[test]
fn agent_loop_path_can_receive_runtime_input_derived_budget_and_typed_policy() {
    let budget = AgentExecutionBudget {
        max_steps: 3,
        max_tool_calls: 2,
        timeout_seconds: 20,
    ...
    let action_ctx = input.attach_policy_to_action_context(base_ctx);
    ...
    assert!(action_ctx.external_write_requires_proposal);
}
...
#[test]
fn runtime_output_life_event_candidates_do_not_persist_to_evidence_store() {
