...
const STATE_STORE_SCHEMA_VERSION: i64 = 11;
...
impl StateStore {
    ...
    fn init_schema(&self) -> Result<()> {
        ...
        let needs_observation_history_backfill = !matches!(
            ...
            None | Some("6") | Some("7") | Some("8") | Some("9") | Some("10") | Some("11")
        );
        ...
        tx.execute_batch(
            "DROP TABLE IF EXISTS state_portable_daily_task_restore_items;
             DROP TABLE IF EXISTS state_portable_daily_task_restore_operations;",
        )?;
        tx.execute(
            "UPDATE state_store_metadata SET value = ?1 WHERE key = 'schema_version'",
            [STATE_STORE_SCHEMA_VERSION.to_string()],
        )?;
...
mod tests {
    ...
    fn schema_v9_rebuilds_restore_provenance_constraints_without_losing_history() {
        ...
        assert_eq!(restore_table_count, 0);
