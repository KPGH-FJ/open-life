...
use openlife_core::vectors::{VectorStore, VectorInsertItem};
...
fn bench_vector_search(c: &mut Criterion) {
    ...
    let embeddings: Vec<Vec<f32>> = (0..1000)
        .map(|i| (0..384).map(|j| ((i * 384 + j) % 100) as f32 / 100.0).collect())
        .collect();
    ...
    c.bench_function("vector_search_1000_chunks", |b| {
        b.iter(|| {
            store.search(black_box(&query), black_box(5)).unwrap()
        })
    });
}
