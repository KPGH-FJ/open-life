...
- **当前 Main Chat 权威路径**：Main Chat Kernel Rescue Goal 8 正在清理旧 Main Chat runtime 重复路径；普通 `send_message` / `start_stream_message` 的默认产品路径必须是 `MainChatKernel` backed，legacy fallback 只能作为显式、可计数、非默认的保留路径。final/readiness gates 必须消费 kernel evidence counters（DirectAnswer、read-only tools、proposal-only writes、PlanExecute draft、blocker、HS context、web/MCP），不能再把旧 `main_chat_strategy.rs` 过度编排路径当作默认权威；文档入口以 `plans/README.md`、`plans/main_chat_agent_kernel_rescue_goal_8_cleanup_final_gate.md` 和 `plans/main_chat_agent_kernel_rescue_acceptance_matrix.md` 为准。
...
- **文档入口**：`plans/README.md` 是文档权威地图；当前 Goal-mode 入口是 `plans/main_chat_agent_kernel_rescue_goal_8_cleanup_final_gate.md`，并以 `plans/main_chat_agent_kernel_rescue_goal_mode_index.md` 与 `plans/main_chat_agent_kernel_rescue_acceptance_matrix.md` 校验 K8。旧 stabilization / migration / productization 文档只作审计参考，不能覆盖 default Main Chat 必须 kernel-backed、legacy fallback 显式可计数、final/readiness gates 消费 kernel evidence counters 的要求。
- **仓库链接**：（需要人工补充）
...
2. [`plans/main_chat_agent_kernel_rescue_goal_8_cleanup_final_gate.md`](plans/main_chat_agent_kernel_rescue_goal_8_cleanup_final_gate.md)：当前 Goal-mode 入口。清理 legacy runtime 重复路径，保持普通 send/stream 默认 `MainChatKernel` backed，并让 final/readiness gates 消费 kernel evidence counters。
...
- 当前 Goal-mode 默认从 Goal 8 cleanup / final-gate realignment 继续；不得默认回到旧 stabilization / migration 大 Goal。除非用户显式启动 `plans/main_chat_agent_migration_v1_goal_spec.md` 定义的 Main Chat Agent Migration v1 Goal，否则后续 Agent 不得把 legacy `main_chat_strategy.rs` 重新提升为 `send_message`、`start_stream_message` 或 Chat 主流程的默认权威；任何保留路径都必须是显式、可计数、非默认 fallback。
