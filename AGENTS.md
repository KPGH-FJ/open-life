...
- **当前阶段**：W23 Controlled Pilot Promotion Evidence Recorder 已完成。MultiStrategy Runtime 仍不是默认 Chat 主链路；Chat 页面现在有用户显式点击的 Controlled Pilot 单轮入口，会先调用 `check_controlled_chat_pilot_eligibility`，通过后才以 `allowWrites=false` 调用 `run_multi_strategy_agent_preview`。成功且包含 `userOutput` 的 Pilot response 可由用户显式 review/confirm 后提升为一条 ordinary assistant message，但 confirmation 必须校验当前 target session 与 pilot source session 一致；session mismatch 不调用 `save_chat_message`，只显示 blocking/fallback 和重新运行 pilot 提示。确认 promotion 且 assistant message 保存成功后，才写入 metadata-safe EvidenceStore promotion evidence；Settings experimental 只读展示 promoted count、recent promoted pilot run ids、latest timestamp 和 mismatch block count。默认 `Send` / `send_message` / `start_stream_message` 保持现状，且不得调用 eligibility、gate、preview、promotion 或 evidence recorder。`make ci` 为发布门控。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W23 完成度与当前 non-default preview / migration gate / pilot eligibility / controlled pilot / reviewed promotion / source-bound validation / promotion evidence 状态索引；不是第二套路线图。
...
- 当前分支已完成 W1-W23；`check_controlled_chat_pilot_eligibility` 可只读判断最近 preview gate evidence 是否连续干净。Chat 页面有 very small Controlled Pilot：必须用户显式点击，先检查 eligibility，blocked 时只展示 blocking reasons/fallback，不调用 preview；eligible 时才运行单次 preview，且强制 `allowWrites=false`。成功 pilot 默认仍隔离；只有用户显式 review/confirm promotion 且 source/target session 一致后，才写入一条 ordinary assistant message；随后只记录 metadata-safe promotion evidence，不持久化 raw pilot response/raw user prompt/full tool payload。默认 Chat 主路径仍不能替换。
...
| **Controlled Chat Pilot / Promotion Evidence** | [`frontend/src/pages/ChatPage.tsx`](frontend/src/pages/ChatPage.tsx) + [`src-tauri/src/commands/agent_runtime.rs`](src-tauri/src/commands/agent_runtime.rs) | W20 very small controlled pilot + W21 reviewed promotion + W22 source-bound validation + W23 evidence recorder：Chat 页面显式按钮先查 eligibility，eligible 后运行单次 write-disabled preview；成功 `userOutput` 可 review/confirm promotion | 默认 Send 不变；blocked/failed/no-output/cancel/repeat 不写入；确认后只在 source/target session 一致时写一条 ordinary assistant message，并写入 metadata-safe promotion evidence，可带 `run_id` trace |
