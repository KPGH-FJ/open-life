...
- LifeModel Maturation Loop End-to-End Goal 必须以 `plans/lifemodel_maturation_goal_plan.md` 为入口；W73 只完成 read-only readiness report，W74 才可实现显式 non-default maturation invocation。不得自动接入 ordinary Chat，不得直接写 LifeModel/Memory/Heuristic active truth，不得绕过 Proposal/Governor，不得扩大到 identity/values/relationships/health/finance 等高风险域。
