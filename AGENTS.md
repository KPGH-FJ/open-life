...
- W54-W56 是 default Chat adapter guard/status 收口：W54 只同步权威路线，W55 只加 ordinary-entry preflight / side-effect lock，W56 只加只读 ordinary-entry preflight status surface。普通 Send / `send_message` / `start_stream_message` 仍只能进入 `legacy_stream`，不得调用 W56 status command，也不能把 `statusReady` 解释为迁移许可。
