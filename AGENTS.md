...
- **当前阶段**：W89 Proposal Application Source-Specific Patch Audit / Readiness 已完成。W64 已完成 W1-W63 文档权威压缩验收；W65-W72 是 default Chat adapter backend-only descriptor/contract/harness/proof/gate/skeleton/binding guard stack；W73-W78 是 LifeModel maturation end-to-end proof slice，停在 metadata-safe trace visibility proof，不运行 runtime/model/tool、不写 AgentRun/LifeModel/Memory/Heuristic truth、不接 default Chat。W79 新增 machine-readable legacy direct-write inventory guard；W80 为 manual LifeModel editor direct save 增加 metadata-safe manual override audit；W81 为 Builder legacy direct apply 增加 fail-closed dev/migration gate 和 no-signal no-write guard；W82 为 Calibration direct/evolution legacy persistence 增加 fail-closed dev/migration gate；W83 为 Feedback evolution legacy direct apply 增加 fail-closed dev/migration gate 并将 report 拆为 read-only；W84 为 Snapshot restore / Data import 增加 legacy direct write gate；W85 在 `src-tauri/src/legacy_write_convergence.rs` 新增 backend-only/internal State/Daily Goal source-data boundary proof；W86 在同一内部模块新增 LifeModel compatibility materializer caller matrix；W87 新增 typed caller purpose/context/restriction report/evaluator/ensure 并要求生产 `persist_life_model` 调用点显式传入 W86 context；W88 在 `src-tauri/src/commands/proposal.rs` 为 accepted LifeModel proposal apply 增加 metadata-safe source-specific PatchSource mapper/report/ensure，并替换 `apply_proposal_to_state` 中硬编码的 `PatchSource::BuilderReview`；W89 在同一文件新增私有 readiness entry/report/evaluator/ensure，证明 exact mappings=4、metadata-safe fallback mappings=5、unsupported_or_unclassified=0、BuilderReview 只用于 BuilderReview、apply path 仍调用 W88 mapping ensure + resolver 并传入 `LifeModelPatch::from_proposal`、default Chat entrypoints 不调用 W88/W89 helper。W89 不新增 command/frontend/Settings surface，不改变 default Chat，不运行 runtime/model/tool，不退休 legacy path，不授予 migration permission 或 runtime authority；它不是 full convergence，fallback source strategy 仍是 blocker，`proposal_first_convergence_complete=false`。
- **Default Chat 硬约束**：default Chat 仍是 `legacy_stream`。普通 `Send` / `send_message` / `start_stream_message` 只能进入 legacy path；允许调用的 adapter 相关代码仅限 W49-W55 共享 pure ordinary-entry guard/preflight，并且只能 fail closed，不能切换路由。普通入口不得调用 W19-W60 command surfaces，也不得调用 W67 non-default invocation harness、W68 send-compatible proof、W69 stream boundary proof、W70 executor attachment gate、W71 disabled executor skeleton、W72 skeleton binding integrity report、W73 LifeModel maturation readiness report、W74 non-default LifeModel maturation invocation、W75 proposal outcome evidence helper、W76 low-energy collaboration rule candidate helper、W77 accepted rule selection helper、W78 trace visibility helper、W79 legacy write convergence inventory guard、W80 manual LifeModel override audit helper、W81 Builder legacy direct apply helper/override、W82 Calibration legacy direct apply helper/override、W83 Feedback evolution legacy direct apply helper/override、W84 snapshot restore / data import legacy direct apply helper/override、W85 State/Daily Goal source-data boundary helper、W86 LifeModel materializer caller matrix helper、W87 LifeModel materializer caller restriction evaluator/ensure helper、W88 proposal PatchSource mapping helper或 W89 proposal PatchSource readiness helper；W87 允许普通 Chat 现有 daily-goal auto-checkin 只向 `persist_life_model` 传 source-data compatibility typed context，这不是路由切换。W19-W60 readiness/review/preview/gate/draft/evidence/status 结果、W67 `harness_ready`、W68 `proof_ready`、W69 `proof_ready`、W70 `gate_report_metadata_ready`、W71 `skeleton_contract_ready`、W72 `binding_integrity_ready`、W77 `selected` proof、W78 `trace_visibility_ready` proof、W79 `inventory_ready` report、W80 audit guard、W81 dev-gate、W82 dev-gate、W83 dev-gate、W84 restore/import gate、W85 state source-data boundary proof、W86 caller matrix、W87 caller restriction、W88 PatchSource mapping 和 W89 readiness report 都不是 migration permission。
- **文档入口**：`plans/README.md` 是文档权威地图，`plans/lifemodel_governed_runtime_progress.md` 是 W1-W89 结构化状态索引。若旧长段仍写 W60 latest、ready/approve 可迁移、或 W61-W89 会影响 default Chat，以本 W89 入口、`plans/README.md` 和 progress index 为准。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W89 结构化状态索引，按 stage id / 名称 / 状态 / command-surface 类型 / read-only-write-disabled-metadata-safe / default Chat 影响 / 下一步依赖整理；不是第二套路线图。
...
- 当前分支已完成 W1-W89；W61-W64 是 docs/index整理和权威入口压缩验收，W65-W72 是 backend-only default Chat adapter guard stack，W73-W78 是 LifeModel maturation proof slice，W79 是 Legacy Direct-Write Convergence machine-readable inventory guard，W80 是 manual LifeModel editor metadata-safe explicit override audit guard，W81 是 Builder legacy direct apply dev-gate / no-signal completion guard，W82 是 Calibration legacy direct apply dev-gate / proposal-first default，W83 是 Feedback evolution legacy direct apply dev-gate / read-only report guard，W84 是 Snapshot restore / Data import legacy direct apply dev/migration/manual gate，W85 是 State / Daily Goal source-data boundary proof，W86 是 LifeModel compatibility materializer caller matrix，W87 是 LifeModel materializer typed caller restriction，W88 是 Proposal Application Source-Specific Patch Mapping，W89 是 Proposal Application Source-Specific Patch Audit / Readiness。W65-W72 只描述并验证 future controlled adapter candidate 的 descriptor / contract / invocation / SendMessageResult-compatible shape / `start_stream_message`-compatible metadata boundary / attachment gate metadata readiness / disabled skeleton placeholder / skeleton binding metadata integrity，不接入 ordinary Chat callsite，不授权迁移；W73-W78 不接入 ordinary Chat，不运行 runtime/model/tool，且不改变 default Chat `legacy_stream`；W79 只盘点 legacy direct-write blockers，不新增 command/frontend，不改变产品行为，不收口 direct writes；W80 只为 manual editor save 增加 metadata-safe audit，不把 manual editor 标记为 proposal-first 或 fully converged；W81 只把 Builder legacy direct apply 默认 fail closed 并证明 no-signal completion 不写 durable LifeModel truth，不把 Builder legacy path 标记为 fully converged；W82 只把 Calibration direct/evolution legacy writes 默认 fail closed 并保持 normal Calibration/Dashboard flow proposal-first，不把 Calibration direct/evolution path 标记为 fully converged；W83 只把 Feedback evolution legacy direct apply 默认 fail closed、`generate_evolution_report` 改为 read-only，不把 Feedback evolution direct apply 标记为 fully converged；W84 只把 restore/import 默认 fail closed 并保持 export/read-only 路径不受影响，不把 restore/import 标记为 fully converged；W85 只证明 State/Daily Goal 当前写 compatibility view 的 source-data / low-risk transient compatibility boundary，不做 proposal-first conversion、不改变 default Chat、不把 State/Daily Goal 标为 fully converged，提升到 durable LifeModel-HS truth 仍必须走 proposal-first；W86 只证明 materializer caller 分类和 metadata-safety；W87 只执行 caller restriction，不改 default Chat、不退休 legacy path、不授予 runtime authority；W88 只修正 accepted LifeModel proposal apply 的 PatchStore/audit source 语义；W89 只证明 W88 mapping readiness、apply path source resolver usage 和 default Chat untouched，不运行 runtime/model/tool、不新增 command/frontend，不把 proposal-first convergence 标记为完成。
