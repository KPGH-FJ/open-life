...
- 当前分支已完成 W1-W26；`check_controlled_chat_pilot_eligibility` 可只读判断最近 preview gate evidence 是否连续干净。Chat 页面有 very small Controlled Pilot：必须用户显式点击，先检查 eligibility，blocked 时只展示 blocking reasons/fallback，不调用 preview；eligible 时才运行单次 preview，且强制 `allowWrites=false`。成功 pilot 默认仍隔离；只有用户显式 review/confirm promotion 且 source/target session 一致后，才写入一条 ordinary assistant message；随后只记录 metadata-safe promotion evidence，不持久化 raw pilot response/raw user prompt/full tool payload。`check_controlled_pilot_promotion_readiness` 只读判断既有 promotion evidence 是否足以进入下一步 Chat migration 讨论；`draft_controlled_chat_migration_plan` 在 readiness 通过后只生成只读人工评审草案，不执行迁移、不改默认 flag、不新增 evidence；`record_controlled_chat_migration_review_decision` 仅记录人工 approve/reject/request_rework 的 metadata-safe decision evidence，blocked draft approve 不写 evidence。默认 Chat 主路径仍不能替换。
...
| **Controlled Chat Pilot / Promotion Evidence / Readiness / Draft / Review Decision** | [`frontend/src/pages/ChatPage.tsx`](frontend/src/pages/ChatPage.tsx) + [`src-tauri/src/commands/agent_runtime.rs`](src-tauri/src/commands/agent_runtime.rs) | W20 very small controlled pilot + W21 reviewed promotion + W22 source-bound validation + W23 evidence recorder + W24 readiness gate + W25 draft plan + W26 review decision evidence：Chat 页面显式按钮先查 eligibility，eligible 后运行单次 write-disabled preview；成功 `userOutput` 可 review/confirm promotion；Settings 可只读检查 promotion evidence readiness、生成 migration plan draft，并记录/汇总 metadata-safe review decision evidence | 默认 Send 不变；blocked/failed/no-output/cancel/repeat 不写入；确认后只在 source/target session 一致时写一条 ordinary assistant message，并写入 metadata-safe promotion evidence，可带 `run_id` trace；readiness pass、migration draft 和 review approval 都不是迁移许可 |
