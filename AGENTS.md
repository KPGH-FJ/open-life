...
- **当前阶段**：W78 LifeModel Maturation Run Trace Visibility Proof 已完成。W64 已完成 W1-W63 文档权威压缩验收；W65 新增纯后端 metadata-safe descriptor / mapper；W66 新增 metadata-safe controlled adapter contract report / evaluator / ensure；W67 新增纯后端 non-default invocation harness；W68 新增纯后端 send-compatible proof/evaluator/ensure，只允许 `SendMessage` callsite ready；W69 新增纯后端 stream-compatible boundary proof/evaluator/ensure，只允许 `StartStreamMessage` callsite ready；W70 新增纯后端 executor attachment gate report/evaluator/ensure；W71 新增 disabled/unattached/no-run executor skeleton contract；W72 新增 skeleton binding integrity report。W73 在 `openlife-core/src/agent/maturation.rs` 新增纯 core `LifeModelMaturationReadinessReport` / evaluator / ensure，只验证低能量/低压力规划偏好成熟化候选是否可进入下一步 non-default invocation。W74 新增显式 non-default maturation invocation harness/report，必须先调用 W73 readiness；readiness blocked 时不写任何 store，ready 时只允许写 EvidenceStore + ProposalStore。W75 在 `openlife-core/src/agent/proposal_outcome.rs` 新增 metadata-safe proposal outcome evidence helper/report，并在 `src-tauri/src/commands/proposal.rs` 的 accept/reject/edit 成功处理后做最小接线：只对 maturation lineage proposal 写 `ProposalOutcome` evidence。W76 在 `openlife-core/src/agent/maturation.rs` 新增纯 core low-energy collaboration rule candidate evaluator/proposer，只聚合 metadata-safe ProposalOutcome evidence，在 ready 时写 pending ProposalStore candidate proposal；不激活 Heuristic、不写 active rule。W77 在同一模块新增 accepted low-energy rule selection proof，只证明已被用户接受的 W76 candidate 可进入 future RuntimeHSPacket metadata-safe planning guidance。W78 在同一模块新增 `LowEnergyRuleTraceVisibilityInput` / `LowEnergyRuleTraceVisibilityReport`、`evaluate_low_energy_rule_trace_visibility` 与 `ensure_low_energy_rule_trace_visibility`，只证明 W77 selected guidance 可被 future runtime/run trace 以 metadata-safe trace metadata 展示；只暴露 guidance summary、ids/hash/count/status/type、policy route proof 和 lineage digest，不运行 runtime/model/tool、不写 AgentRun/LifeModel/Memory/Heuristic truth、不接 default Chat。
- **Default Chat 硬约束**：default Chat 仍是 `legacy_stream`。普通 `Send` / `send_message` / `start_stream_message` 只能进入 legacy path；允许调用的 adapter 相关代码仅限 W49-W55 共享 pure ordinary-entry guard/preflight，并且只能 fail closed，不能切换路由。普通入口不得调用 W19-W60 command surfaces，也不得调用 W67 non-default invocation harness、W68 send-compatible proof、W69 stream boundary proof、W70 executor attachment gate、W71 disabled executor skeleton、W72 skeleton binding integrity report、W73 LifeModel maturation readiness report、W74 non-default LifeModel maturation invocation、W75 proposal outcome evidence helper、W76 low-energy collaboration rule candidate helper、W77 accepted rule selection helper 或 W78 trace visibility helper；W19-W60 readiness/review/preview/gate/draft/evidence/status 结果、W67 `harness_ready`、W68 `proof_ready`、W69 `proof_ready`、W70 `gate_report_metadata_ready`、W71 `skeleton_contract_ready`、W72 `binding_integrity_ready`、W77 `selected` proof 和 W78 `trace_visibility_ready` proof 都不是 migration permission。
- **文档入口**：`plans/README.md` 是文档权威地图，`plans/lifemodel_governed_runtime_progress.md` 是 W1-W78 结构化状态索引。若旧长段仍写 W60 latest、ready/approve 可迁移、或 W61-W78 会影响 default Chat，以本 W78 入口、`plans/README.md` 和 progress index 为准。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W78 结构化状态索引，按 stage id / 名称 / 状态 / command-surface 类型 / read-only-write-disabled-metadata-safe / default Chat 影响 / 下一步依赖整理；不是第二套路线图。
...
- 当前分支已完成 W1-W78；W61-W64 是 docs/index整理和权威入口压缩验收，W65 是 backend-only descriptor skeleton，W66 是 backend-only controlled contract report，W67 是 backend-only non-default invocation harness，W68 是 backend-only send-compatible contract proof，W69 是 backend-only stream-compatible boundary proof，W70 是 backend-only executor attachment gate report，W71 是 backend-only disabled executor skeleton contract，W72 是 backend-only disabled executor skeleton binding integrity report，W73 是 LifeModel maturation readiness report，W74 是 explicit non-default LifeModel maturation invocation harness/report，W75 是 maturation proposal outcome evidence link，W76 是 low-energy collaboration rule candidate，W77 是 accepted low-energy rule selection proof，W78 是 metadata-safe trace visibility proof。W65-W72 只描述并验证 future controlled adapter candidate 的 descriptor / contract / invocation / SendMessageResult-compatible shape / `start_stream_message`-compatible metadata boundary / attachment gate metadata readiness / disabled skeleton placeholder / skeleton binding metadata integrity，不接入 ordinary Chat callsite，不授权迁移；W73-W78 不接入 ordinary Chat，不运行 runtime/model/tool，且不改变 default Chat `legacy_stream`。
