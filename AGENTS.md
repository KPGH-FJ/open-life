...
- **当前阶段**：W24 Promotion Evidence Readiness Gate 已完成。MultiStrategy Runtime 仍不是默认 Chat 主链路；Chat 页面现在有用户显式点击的 Controlled Pilot 单轮入口，会先调用 `check_controlled_chat_pilot_eligibility`，通过后才以 `allowWrites=false` 调用 `run_multi_strategy_agent_preview`。成功且包含 `userOutput` 的 Pilot response 可由用户显式 review/confirm 后提升为一条 ordinary assistant message，但 confirmation 必须校验当前 target session 与 pilot source session 一致；session mismatch 不调用 `save_chat_message`，只显示 blocking/fallback 和重新运行 pilot 提示。确认 promotion 且 assistant message 保存成功后，才写入 metadata-safe EvidenceStore promotion evidence；Settings experimental 只读展示 promoted count、recent promoted pilot run ids、latest timestamp、mismatch block count 和 W24 promotion readiness。`check_controlled_pilot_promotion_readiness` 只读取既有 promotion evidence，默认要求 3 条 metadata-safe promotions，`sessionId` 当前按 global summary 说明；gate pass 只表示可进入下一阶段讨论，不是自动迁移许可。默认 `Send` / `send_message` / `start_stream_message` 保持现状，且不得调用 eligibility、gate、preview、promotion、evidence recorder 或 promotion readiness gate。`make ci` 为发布门控。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W24 完成度与当前 non-default preview / migration gate / pilot eligibility / controlled pilot / reviewed promotion / source-bound validation / promotion evidence / promotion readiness 状态索引；不是第二套路线图。
...
| **Controlled Chat Pilot / Promotion Evidence / Readiness** | [`frontend/src/pages/ChatPage.tsx`](frontend/src/pages/ChatPage.tsx) + [`src-tauri/src/commands/agent_runtime.rs`](src-tauri/src/commands/agent_runtime.rs) | W20 very small controlled pilot + W21 reviewed promotion + W22 source-bound validation + W23 evidence recorder + W24 readiness gate：Chat 页面显式按钮先查 eligibility，eligible 后运行单次 write-disabled preview；成功 `userOutput` 可 review/confirm promotion；Settings 可只读检查 promotion evidence readiness | 默认 Send 不变；blocked/failed/no-output/cancel/repeat 不写入；确认后只在 source/target session 一致时写一条 ordinary assistant message，并写入 metadata-safe promotion evidence，可带 `run_id` trace；readiness pass 不是迁移许可 |
