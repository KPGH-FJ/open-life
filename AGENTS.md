...
- **当前阶段**：W80 Manual LifeModel Editor Explicit Override Audit Guard 已完成。W64 已完成 W1-W63 文档权威压缩验收；W65-W72 是 default Chat adapter backend-only descriptor/contract/harness/proof/gate/skeleton/binding guard stack；W73-W78 是 LifeModel maturation end-to-end proof slice，停在 metadata-safe trace visibility proof，不运行 runtime/model/tool、不写 AgentRun/LifeModel/Memory/Heuristic truth、不接 default Chat。W79 在 `src-tauri/src/legacy_write_convergence.rs` 新增 machine-readable inventory guard，明确 `overall_converged=false` / `all_direct_writes_converged=false`。W80 在 `src-tauri/src/commands/life_model.rs` 为 manual LifeModel editor `save_life_model` 新增 `ManualLifeModelOverrideAuditReport`、`evaluate_manual_lifemodel_override_audit` 与 `record_manual_lifemodel_override_audit_with_state`，保存成功后写入 metadata-safe manual override audit：只记录 source、before/after hash、rough changed sections、risk class、timestamp、command、manualOverride/proposalFirst/stillLegacyDirectWrite flags；不写 raw LifeModel 内容，不创建 Proposal/AgentRun/Heuristic，不运行 runtime/model/tool，不接 default Chat。manual editor 仍是 high-risk legacy direct-write blocker，尚未 proposal-first converged。
- **Default Chat 硬约束**：default Chat 仍是 `legacy_stream`。普通 `Send` / `send_message` / `start_stream_message` 只能进入 legacy path；允许调用的 adapter 相关代码仅限 W49-W55 共享 pure ordinary-entry guard/preflight，并且只能 fail closed，不能切换路由。普通入口不得调用 W19-W60 command surfaces，也不得调用 W67 non-default invocation harness、W68 send-compatible proof、W69 stream boundary proof、W70 executor attachment gate、W71 disabled executor skeleton、W72 skeleton binding integrity report、W73 LifeModel maturation readiness report、W74 non-default LifeModel maturation invocation、W75 proposal outcome evidence helper、W76 low-energy collaboration rule candidate helper、W77 accepted rule selection helper、W78 trace visibility helper、W79 legacy write convergence inventory guard 或 W80 manual LifeModel override audit helper；W19-W60 readiness/review/preview/gate/draft/evidence/status 结果、W67 `harness_ready`、W68 `proof_ready`、W69 `proof_ready`、W70 `gate_report_metadata_ready`、W71 `skeleton_contract_ready`、W72 `binding_integrity_ready`、W77 `selected` proof、W78 `trace_visibility_ready` proof、W79 `inventory_ready` report 和 W80 audit guard 都不是 migration permission。
- **文档入口**：`plans/README.md` 是文档权威地图，`plans/lifemodel_governed_runtime_progress.md` 是 W1-W80 结构化状态索引。若旧长段仍写 W60 latest、ready/approve 可迁移、或 W61-W80 会影响 default Chat，以本 W80 入口、`plans/README.md` 和 progress index 为准。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W80 结构化状态索引，按 stage id / 名称 / 状态 / command-surface 类型 / read-only-write-disabled-metadata-safe / default Chat 影响 / 下一步依赖整理；不是第二套路线图。
