...
- 当前分支已完成 W1-W60；W59 只是 W58 narrow implementation plan draft 的人工 review evidence，不迁移 default Chat。后续若本节旧 W1-W58 长段与 W59 状态冲突，以 W58/W59 覆盖更新、`plans/README.md` 和 `plans/lifemodel_governed_runtime_progress.md` 为准；默认 Chat 主路径仍不能替换。
...
- W58 是 default Chat adapter narrow implementation plan draft：Settings 可显式生成只读 human-review plan sections 与 stable digest，但普通 Send / `send_message` / `start_stream_message` 不得调用该 command，`draftReady` 也不是迁移许可。
- W59 是 default Chat adapter narrow implementation plan review evidence：Settings 可显式记录 approve/reject/request_rework metadata-safe evidence 并读取 summary；blocked W58 draft 的 approve 不写 evidence，reviewer note 只保存 checksum/length/category。普通 Send / `send_message` / `start_stream_message` 不得调用 W59 commands，approval 也不是迁移许可。
- W60 是 default Chat adapter narrow implementation plan approval readiness gate：Settings 可显式只读检查当前 W58 draft 与 W59 latest approve evidence 是否仍 digest match，并确认 W57/default Chat isolation 仍干净。普通 Send / `send_message` / `start_stream_message` 不得调用 W60 command，ready 也不是迁移许可。
