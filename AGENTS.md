...
- **当前阶段**：W57 Default Chat Adapter Narrow Implementation Discussion Gate 已完成。W56 Default Chat Adapter Ordinary Entry Preflight Status 也已完成。W55 Default Chat Adapter Ordinary Entry Preflight、W54 Authority Roadmap Sync 与 W53 Default Chat Adapter Typed Callsite Contract 也已完成。MultiStrategy Runtime 仍不是默认 Chat 主链路；Chat 页面现在有用户显式点击的 Controlled Pilot 单轮入口，会先调用 `check_controlled_chat_pilot_eligibility`，通过后才以 `allowWrites=false` 调用 `run_multi_strategy_agent_preview`。成功且包含 `userOutput` 的 Pilot response 可由用户显式 review/confirm 后提升为一条 ordinary assistant message，但 confirmation 必须校验当前 target session 与 pilot source session 一致；session mismatch 不调用 `save_chat_message`，只显示 blocking/fallback 和重新运行 pilot 提示。确认 promotion 且 assistant message 保存成功后，才写入 metadata-safe EvidenceStore promotion evidence；Settings experimental 只读展示 promoted count、recent promoted pilot run ids、latest timestamp、mismatch block count、W24 promotion readiness、W25 Draft Migration Plan、W26 Migration Review Decision summary、W27 Implementation Gate、W28 Shadow Run、W29 Shadow Review、W30 Cutover Readiness、W31 Cutover Candidate、W32 Cutover Candidate Review、W33 Candidate Promotion Readiness、W34 Default Chat Runtime Boundary、W35 Default Chat Adapter Activation Plan、W36 Activation Review Decision、W37 Activation Implementation Gate、W38 Adapter Routing Status、W39 Adapter Contract Harness、W40 Adapter Dry Run、W41 Adapter Dry Run Review、W42 Adapter Implementation Readiness、W43 Adapter Controlled Preview、W44 Adapter Controlled Preview Review、W45 Adapter Controlled Preview Approval Readiness、W46 Adapter Cutover Implementation Plan、W47 Adapter Cutover Plan Review、W48 Adapter Cutover Plan Approval Readiness、W49 Adapter Route Guard Scaffold、W50 Adapter Cutover Invocation Harness、W51 Adapter Invocation Plan、W52 Adapter Invocation Boundary、W53 Adapter Typed Callsite Contract、W54 Authority Roadmap Sync、W55 Adapter Ordinary Entry Preflight、W56 Adapter Ordinary Entry Preflight Status 和 W57 Adapter Narrow Implementation Discussion Gate。W54 只同步高优先级路线文档到 W1-W53 当前代码状态。W55 只新增纯后端 ordinary-entry preflight / side-effect lock，默认 `send_message` / `start_stream_message` 进入 `legacy_stream` 前必须满足 typed contract ready、legacy entry allowed、controlled executor unattached、default migration disabled 和零 runtime/model/tool/write 预入口预算；它不修改 controlled runtime，不是 default Chat migration。W56 只新增只读 ordinary-entry preflight status command、frontend wrapper 与 Settings panel，只读取当前 route 和 W55 preflight，返回 metadata-safe summary；它不运行 runtime/model/tool，不写记录，不是 default Chat migration。W57 只新增只读 narrow implementation discussion gate、frontend wrapper 与 Settings panel，组合 W48 cutover plan approval readiness 与 W56 ordinary-entry preflight status；eligible 只表示可讨论更窄 adapter implementation slice，不运行 runtime/model/tool，不写记录，不切换 routing，不是 default Chat migration。`check_controlled_pilot_promotion_readiness` 只读取既有 promotion evidence，默认要求 3 条 metadata-safe promotions，`sessionId` 当前按 global summary 说明；gate pass 只表示可进入下一阶段讨论，不是自动迁移许可。`draft_controlled_chat_migration_plan` 复用 readiness gate：blocked 时不生成可执行 plan，passed 时只返回人工评审草案，并固定 `manualReviewRequired=true`、`notAutomaticMigration=true`。`record_controlled_chat_migration_review_decision` 会先调用 draft command，只在 metadata-safe EvidenceStore 中记录显式 approve/reject/request_rework 决策；blocked draft 的 approve 不写 evidence，reviewer note 只保存 length/checksum/category。`check_controlled_chat_migration_implementation_gate` 只读读取当前 readiness、当前 draft hash 和 latest metadata-safe review decision evidence；只有 latest approve、readiness pass、hash match 才 eligible。`run_controlled_chat_migration_shadow_run` 是显式非默认 shadow command：先查 implementation gate，blocked 不执行 runtime，eligible 后才以 bounded descriptor 运行 `allowWrites=false` controlled runtime preview，并只返回 metadata-safe strategy/payload/summary/warnings/blockers。它可写 metadata-safe shadow AgentRun audit，但不写 Chat message、Proposal、Memory、LifeModel patch、Evidence 或外部工具结果。`record_controlled_chat_migration_shadow_review_decision` 只记录人工 approve/reject/request_rework shadow review evidence；任何 decision 都必须绑定已完成且 metadata-safe、write-disabled、无副作用的 `controlled_migration_shadow_run` AgentRun，Evidence metadata 只保存 shadowRunId、decisionKind、reviewerNote checksum/length/category、readinessSummaryDigest 和 createdAt。`check_controlled_chat_cutover_readiness` 是只读 cutover planning readiness gate：要求 W27 当前 eligible、latest W29 shadow review decision 为 approve、approved shadowRunId 对应 AgentRun 仍存在且 completed / `controlled_migration_shadow_run` / `allowWrites=false` / `metadataSafe=true` / 无副作用，并只返回 metadata-safe summary。`run_controlled_chat_cutover_candidate` 是显式非默认 candidate command：先调用 W30 readiness，blocked 时不运行 runtime；eligible 后才执行一次 `allowWrites=false`、`maxToolCalls=0` 的 controlled runtime candidate，返回 Chat-compatible contract shape 字段，并只允许 metadata-safe AgentRun audit，不保存 raw prompt/output/tool payload，不写 Chat/Proposal/Memory/LifeModel/Evidence/MCP audit/外部工具结果。`record_controlled_chat_cutover_candidate_review_decision` 和 `get_controlled_chat_cutover_candidate_review_summary` 只服务人工审阅 W31 candidate 结果；approve 必须绑定已完成、`reasoning_strategy=controlled_chat_cutover_candidate`、`contractShape=send_message_compatible`、`candidateReady=true`、`allowWrites=false`、`maxToolCalls=0`、`metadataSafe=true` 且无 Chat/Proposal/Memory/LifeModel/Evidence/MCP audit/external write 副作用的 AgentRun；review evidence metadata 只保存 candidateRunId、decisionKind、contractShape、candidateSummaryDigest、reviewerNote checksum/length/category 和 createdAt。`check_controlled_chat_cutover_candidate_promotion_readiness` 只读检查 W30 readiness、W32 metadata-safe approval evidence、latest decision、approved candidate AgentRun 当前安全状态和 default Chat isolation；它不创建记录、不运行 runtime/tool/model call。`get_default_chat_runtime_boundary_status` 只读声明默认 Chat 仍是 `legacy_stream`，automatic migration disabled，controlled candidate not available on default path；它不调用 W19-W33 gates，不创建记录，不运行 runtime/tool/model call。`draft_default_chat_adapter_activation_plan` 只读组合 W33 readiness 与 W34 boundary status：blocked 时不生成 plan sections，ready 时仅返回 human-review activation plan，并固定 `manualReviewRequired=true`、`notAutomaticMigration=true`、`requiresSeparateImplementation=true`；它不创建记录、不运行 runtime/tool/model call、不切换 feature flag。`record_default_chat_adapter_activation_review_decision` 会先调用 W35 draft；blocked draft approve 不写 evidence；ready draft 可记录 approve/reject/request_rework metadata-safe evidence，reviewer note 仅保存 checksum/length/category；summary 只读。`check_default_chat_adapter_activation_implementation_gate` 只读组合当前 W35 stable activation plan digest 与 W36 latest metadata-safe review evidence；只有 latest approve、draft ready、digest match、candidate promotion ready、default Chat 仍为 `legacy_stream` 且 automatic migration disabled 时才 eligible；它不创建记录、不运行 runtime/tool/model call、不切换 feature flag。`get_default_chat_adapter_routing_status` 只读调用 W37 gate，并固定声明 adapter scaffold present 但 controlled adapter disabled、defaultSendPath/startStreamPath 均为 `legacy_stream`；它不创建记录、不运行 runtime/tool/model call、不切换 routing。`check_default_chat_adapter_contract_harness` 只读调用 W38 routing status，检查 send_message/start_stream_message contract 仍为 `legacy_stream`、controlled adapter disabled、activation implementation gate eligible，并返回 metadata-safe contract checks；它不创建记录、不运行 runtime/tool/model call、不切换 routing。`run_default_chat_adapter_dry_run` 是显式非默认 dry-run command：先调用 W39 contract harness；blocked 时不运行 dry run，ready 时只返回 metadata-safe invocation contract result，并强制 `allowWrites=false`、`maxToolCalls=0`、`defaultChatPathUnchanged=true`；它不保存 Chat、不创建 AgentRun/Evidence/Proposal/Memory/LifeModel/MCP audit/external write、不运行 runtime/tool/model call、不切换 routing。`record_default_chat_adapter_dry_run_review_decision` 会先重新运行 W40 dry run；approve 只在 dry run ready 时写 metadata-safe evidence，blocked approve 不写 evidence，reject/request_rework 只写白名单 metadata，reviewer note 仅保存 checksum/length/category；summary 只读。`check_default_chat_adapter_implementation_readiness` 只读组合 W37/W39/W40/W41 当前证据；只有 activation implementation gate eligible、contract harness ready、dry run ready、latest dry-run review approve、当前 dry-run digest match、default Chat unchanged、controlled adapter disabled、automatic migration disabled、send/stream 均保持 `legacy_stream` 时才 ready；它不创建记录、不运行 runtime/tool/model call、不切换 routing。`run_default_chat_adapter_controlled_preview` 是显式非默认 controlled preview command：先调用 W42 implementation readiness；blocked 时不运行 runtime、不创建 AgentRun；ready 后才以 `allowWrites=false`、`maxToolCalls=0` 运行一次 controlled preview，返回 SendMessageResult-compatible fields，并只允许 metadata-safe adapter preview AgentRun audit；它不保存 Chat、不写 Evidence/Proposal/Memory/LifeModel/MCP audit/external write、不切换 routing。`record_default_chat_adapter_controlled_preview_review_decision` 和 `get_default_chat_adapter_controlled_preview_review_summary` 只服务人工审阅 W43 preview；approve 必须绑定 completed / send-message-compatible / previewReady / write-disabled / zero-tool / metadata-safe / side-effect-free preview AgentRun，reject/request_rework 也只写白名单 metadata，reviewer note 仅保存 checksum/length/category，summary 只读。`check_default_chat_adapter_controlled_preview_approval_readiness` 只读组合 W42 readiness、W44 latest approve evidence、required approved preview count、digest match 与 approved W43 preview AgentRun 当前安全状态；它不创建记录、不运行 controlled preview/runtime/tool/model call、不切换 routing。`draft_default_chat_adapter_cutover_implementation_plan` 只读调用 W45 readiness；blocked 时不生成 plan sections，ready 时仅返回 metadata-safe human-review cutover implementation plan sections 和 stable plan digest；它不创建记录、不运行 controlled preview/runtime/tool/model call、不切换 routing。`record_default_chat_adapter_cutover_plan_review_decision` 会先调用 W46 draft；blocked draft approve 不写 evidence；ready draft 可记录 approve/reject/request_rework metadata-safe evidence，reviewer note 仅保存 checksum/length/category；summary 只读。`check_default_chat_adapter_cutover_plan_approval_readiness` 只读组合当前 W46 draft、W47 latest review approval、plan digest match、W45 readiness 与 default Chat isolation；ready 也只是后续 adapter implementation discussion readiness；它不创建记录、不运行 runtime/tool/model call、不切换 routing。W49 `default_chat_adapter` route guard 是纯后端 fail-closed 守卫：默认 `send_message` / `start_stream_message` 可以调用它确认当前仍为 `legacy_stream`，但不得调用 W19-W48 gates，不启用 controlled adapter，不自动迁移。W50 `default_chat_adapter` cutover invocation harness 是纯后端 guard：默认 `send_message` / `start_stream_message` 只允许 `legacy_guarded`、write-disabled、zero-tool、no-runtime/no-model/no-tool/no-business-write 边界，并在 route drift 或 adapter 误启用时 fail closed。W51 `default_chat_adapter` invocation plan 是纯后端 guard：默认 `send_message` / `start_stream_message` 只选择 `legacy_stream`，保留 `controlled_adapter` 为 disabled candidate，controlled executor unattached，且 W50 harness blocking 会让 plan blocking。eligible/shadow ready/shadow review approval/cutover planning eligible/candidate ready/candidate review approval/candidate promotion readiness/default Chat boundary status/activation plan draft/activation review approval/activation implementation gate eligible/adapter routing status/contract harness ready/dry-run ready/dry-run review approval/implementationReady/previewReady/controlled preview review approval/controlled preview approval readiness/cutover implementation plan draft/cutover plan review approval/cutover plan approval readiness/route guard/cutover invocation harness/invocation plan/invocation boundary/typed callsite contract/ordinary-entry preflight/ordinary-entry preflight status/narrow implementation discussion gate 只允许对比、人工审阅、进入默认 Chat 迁移实现讨论、验证 contract shape、观察 boundary、activation planning、implementation gate discussion、separate implementation discussion、disabled scaffold observation、adapter contract checking、write-disabled invocation checking、controlled preview、controlled preview review、approval readiness、cutover implementation planning、cutover plan review、pre-entry side-effect locking、只读 preflight status observation、narrow implementation discussion 或后续 adapter 实现讨论，不是 Chat migration。默认 `Send` / `send_message` / `start_stream_message` 保持 `legacy_stream`，只允许调用共享 pure ordinary-entry preflight guard，且不得调用 eligibility、gate、preview、promotion、evidence recorder、promotion readiness gate、migration draft、migration review decision、implementation gate、shadow run、shadow review、cutover readiness、cutover candidate、cutover candidate review、candidate promotion readiness、default Chat boundary status、activation plan draft、activation review decision、activation implementation gate、adapter routing status、contract harness、ordinary-entry preflight status、narrow implementation discussion gate、dry-run command、dry-run review command、implementation readiness command、controlled preview command、controlled preview review command、controlled preview approval readiness command、cutover implementation plan draft、cutover plan review command 或 cutover plan approval readiness command。`make ci` 为发布门控。
- **仓库链接**：（需要人工补充）
...
3. [`plans/lifemodel_governed_runtime_progress.md`](plans/lifemodel_governed_runtime_progress.md)：W1-W57 完成度与当前 non-default preview / migration gate / pilot eligibility / controlled pilot / reviewed promotion / source-bound validation / promotion evidence / promotion readiness / reviewed migration plan draft / manual review decision evidence / implementation gate / shadow run / shadow review evidence / cutover planning readiness / cutover candidate adapter / cutover candidate review evidence / candidate promotion readiness / default Chat runtime boundary / activation plan draft / activation review evidence / activation implementation gate / disabled routing scaffold / contract harness / dry-run boundary / dry-run review evidence / implementation readiness / controlled preview / controlled preview review evidence / controlled preview approval readiness / cutover implementation plan draft / cutover plan review evidence / cutover plan approval readiness / route guard scaffold / cutover invocation harness / invocation plan / invocation boundary / typed callsite contract / authority roadmap sync / ordinary-entry preflight / ordinary-entry preflight status / narrow implementation discussion gate 状态索引；不是第二套路线图。
...
- 当前分支已完成 W1-W57；`check_controlled_chat_pilot_eligibility` 可只读判断最近 preview gate evidence 是否连续干净。Chat 页面有 very small Controlled Pilot：必须用户显式点击，先检查 eligibility，blocked 时只展示 blocking reasons/fallback，不调用 preview；eligible 时才运行单次 preview，且强制 `allowWrites=false`。成功 pilot 默认仍隔离；只有用户显式 review/confirm promotion 且 source/target session 一致后，才写入一条 ordinary assistant message；随后只记录 metadata-safe promotion evidence，不持久化 raw pilot response/raw user prompt/full tool payload。W54 只是 authority roadmap sync，不改变任何 runtime 行为；W55 只是 ordinary-entry preflight / side-effect lock，不迁移 default Chat；W56 只是只读 ordinary-entry preflight status surface，不迁移 default Chat；W57 只是只读 narrow implementation discussion gate，不迁移 default Chat。`check_controlled_pilot_promotion_readiness` 只读判断既有 promotion evidence 是否足以进入下一步 Chat migration 讨论；`draft_controlled_chat_migration_plan` 在 readiness 通过后只生成只读人工评审草案，不执行迁移、不改默认 flag、不新增 evidence；`record_controlled_chat_migration_review_decision` 仅记录人工 approve/reject/request_rework 的 metadata-safe decision evidence，blocked draft approve 不写 evidence；`check_controlled_chat_migration_implementation_gate` 只读判断是否具备进入 controlled Chat migration implementation discussion 的资格，不执行迁移、不新增 evidence；`run_controlled_chat_migration_shadow_run` 只有在 implementation gate eligible 后才执行 write-disabled bounded shadow runtime，可写 metadata-safe shadow AgentRun audit，但不写 Chat/Proposal/Memory/LifeModel/Evidence/外部工具结果；`record_controlled_chat_migration_shadow_review_decision` 只记录 metadata-safe shadow review evidence，字段白名单固定且不保存 reviewer 原文、shadow prompt/output 或 tool payload；`check_controlled_chat_cutover_readiness` 只读判断是否可进入默认 Chat 迁移实现讨论，不运行 runtime、不写 evidence、不迁移默认 Chat；`run_controlled_chat_cutover_candidate` 只在 W30 eligible 后显式运行一次 write-disabled、zero-tool candidate，返回 contract-shaped metadata-safe result，不保存到 Chat，不 promotion，不改默认路径；`record_controlled_chat_cutover_candidate_review_decision` 只记录人工 candidate review evidence，字段白名单固定且不保存 reviewer 原文、candidate userOutput、raw prompt/output 或 tool payload；`check_controlled_chat_cutover_candidate_promotion_readiness` 只读判断 W30/W32 证据和 approved candidate AgentRun 当前状态是否足以进入后续 adapter boundary / activation planning，不创建记录、不运行 runtime/tool/model call；`get_default_chat_runtime_boundary_status` 只读声明默认 Chat 仍是 `legacy_stream`，automatic migration disabled，不调用 W19-W33 gates；`draft_default_chat_adapter_activation_plan` 只读组合 W33 readiness 与 W34 boundary status，blocked 时不生成 plan sections，ready 时只返回人工 activation plan draft；`record_default_chat_adapter_activation_review_decision` 只记录人工 activation review evidence，blocked draft approve 不写 evidence，summary 只读；`check_default_chat_adapter_activation_implementation_gate` 只读检查 W35 stable activation plan digest 与 W36 latest metadata-safe approve evidence 是否仍匹配，eligible 也只是 separate implementation discussion readiness；`get_default_chat_adapter_routing_status` 只读观察 adapter scaffold present 但 controlled adapter disabled，defaultSendPath/startStreamPath 均保持 legacy stream；`check_default_chat_adapter_contract_harness` 只读验证 send/stream contract 均保持 legacy stream，controlled adapter disabled；`run_default_chat_adapter_dry_run` 仅显式返回 write-disabled invocation contract result，不保存 Chat、不运行 runtime/tool/model call；`record_default_chat_adapter_dry_run_review_decision` 只记录 metadata-safe dry-run review evidence，approve 需要 dry run ready，summary 只读；`check_default_chat_adapter_implementation_readiness` 只读组合 W37/W39/W40/W41 证据，ready 也只是 adapter implementation coding discussion readiness；`run_default_chat_adapter_controlled_preview` 先查 W42 readiness，blocked 不运行 runtime/不创建 AgentRun，ready 后运行 write-disabled/zero-tool controlled preview，只写 metadata-safe adapter preview AgentRun audit；`record_default_chat_adapter_controlled_preview_review_decision` 只记录 metadata-safe controlled preview review evidence，approve 需要 completed/ready/send-compatible/write-disabled/zero-tool/metadata-safe/side-effect-free preview AgentRun，summary 只读；`check_default_chat_adapter_controlled_preview_approval_readiness` 只读组合 W42/W44 证据和 approved preview AgentRun 当前安全状态，ready 也只是后续 adapter cutover implementation discussion readiness；`draft_default_chat_adapter_cutover_implementation_plan` 只读组合 W45 readiness，blocked 时不生成 plan sections，ready 时只返回 metadata-safe human-review cutover implementation plan draft，draftReady 不是默认 Chat migration；`record_default_chat_adapter_cutover_plan_review_decision` 只记录人工 cutover plan review evidence，blocked draft approve 不写 evidence，summary 只读；`check_default_chat_adapter_cutover_plan_approval_readiness` 只读检查 W46/W47 证据、当前 plan digest match 与 default Chat isolation，ready 也只是后续 adapter implementation discussion readiness。默认 Chat 主路径仍不能替换。
- W49 `default_chat_adapter` route guard 是纯后端 fail-closed 守卫：默认 `send_message` / `start_stream_message` 可以调用它确认当前仍为 `legacy_stream`，但不得调用 W19-W48 gates，不启用 controlled adapter，不自动迁移，不写任何业务数据。W50 `default_chat_adapter` cutover invocation harness 是纯后端 guard：默认 `send_message` / `start_stream_message` 只允许 `legacy_guarded`、write-disabled、zero-tool、no-runtime/no-model/no-tool/no-business-write 边界，并在 route drift 或 adapter 误启用时 fail closed。W51 `default_chat_adapter` invocation plan 是纯后端 guard：默认 `send_message` / `start_stream_message` 只选择 `legacy_stream`，保留 `controlled_adapter` 为 disabled candidate，controlled executor unattached，且 W50 harness blocking 会让 plan blocking。W52 `default_chat_adapter` invocation boundary 是纯后端 guard：默认 `send_message` / `start_stream_message` 只允许进入 `legacy_stream` callsite，要求 legacy entry 前 write-disabled、zero-tool、no-runtime/no-model/no-tool/no-business-write，且 W51 plan blocking 会让 boundary blocking。W53 `default_chat_adapter` typed callsite contract 是纯后端 guard：默认 `send_message` / `start_stream_message` 必须通过类型化 callsite 绑定 send/stream contract shape 和各自 legacy route path，route drift 会 fail closed。W54 是 authority roadmap sync：若高优先级计划文件与代码状态冲突，以 W54 后的入口文档和 progress index 为准，并先修正文档。W55 `default_chat_adapter` ordinary-entry preflight 是纯后端 guard：默认 `send_message` / `start_stream_message` 只能在 typed contract ready、legacy entry allowed、controlled executor unattached、migration disabled 和零副作用预算下进入 `legacy_stream`。W56 `get_default_chat_adapter_ordinary_entry_preflight_status` 是只读 status surface：Settings 可显式刷新 send/stream W55 preflight 状态，但普通 Send 路径不得调用它，statusReady 也不是迁移许可。W57 `check_default_chat_adapter_narrow_implementation_discussion_gate` 是只读 discussion gate：Settings 可显式检查 W48/W56 当前状态是否足以讨论更窄 adapter implementation slice，但普通 Send 路径不得调用它，eligible 也不是迁移许可。
...
- W20 Controlled Pilot 是 Chat 页面上的显式、单次、可回退路径：普通 Send 不调用 eligibility/gate/preview；pilot blocked/failed 时显示 fallback，不自动重试。W21 Reviewed Pilot Response Promotion 只对成功且包含 `userOutput` 的 pilot 显示操作，必须用户确认后才通过现有 chat message save path 写入一条 assistant message；W22 要求 promotion 绑定 source session 并在确认前校验 target session，一旦 mismatch 不写入并提示重新运行 pilot。W23 在成功 promotion 后记录 metadata-safe evidence，字段限于 pilotRunId、source/target session、strategy/payload/governance、message length、checksum 和 promotedAt；evidence 失败必须显示 degraded/error，重试不得重复写 chat message。W24 readiness gate 只读读取 promotion evidence，不创建 AgentRun/Proposal/Action/Observation，不写 LifeModel/Memory/外部工具；ready 只表示可讨论下一步。W25 migration draft 复用 W24 readiness，blocked 时不生成 plan sections，passed 时只生成人工评审草案；它不创建 AgentRun/Proposal/Memory/LifeModel patch/promotion evidence，也不输出 raw user content、raw assistant output 或 tool payload。W26 review decision evidence 先调用 W25 draft，记录 approve/reject/request_rework 的 metadata-safe evidence；draft blocked 时 approve 被拒绝且不写 evidence，reviewer note 不原文存储。W27 implementation gate 只读读取 W24/W25/W26 结果，latest reject/request_rework、hash mismatch 或当前 readiness blocked 均阻断；eligible 也不会切换 default Chat。W28 shadow run 是显式非默认对比入口，blocked gate 不执行 runtime，eligible 后 runtime 强制 `allowWrites=false` 且输出 metadata-safe；取消、blocked、failed、no-output、重复 promotion 均不写入，不写 LifeModel/Memory/Proposal/外部工具结果。W29 shadow review 只允许人工记录 shadow review evidence；approve 需要 shadow AgentRun completed、write-disabled、metadata-safe、无副作用；reject/request_rework 也只保存 metadata-safe evidence 字段，不保存原文或 tool payload。W30 cutover readiness 只读检查 W27 + latest W29 approve + approved shadow run 仍安全，pass 只表示可进入默认 Chat 迁移实现讨论，不是迁移许可。W31 cutover candidate 是显式非默认 adapter，只在 W30 eligible 后运行一次 zero-tool/write-disabled candidate，用于验证 contract shape，不保存到 Chat，不 promotion，不自动调用。W32 candidate review evidence 只允许 Settings/command 显式记录 approve/reject/request_rework 和只读 summary；approve 绑定 ready candidate AgentRun，证据只保存白名单 metadata，不保存 reviewer 原文、candidate output、raw prompt/output 或 tool payload。W33 candidate promotion readiness 只读检查 W30/W32 证据和 approved candidate run 当前状态，ready 也只是 implementation-planning readiness。W34 default Chat runtime boundary status 只读声明默认 Chat 仍是 legacy stream path，automatic migration disabled；W35 activation plan draft 只读生成 human-review-only activation plan sections；W36 activation review 只记录 metadata-safe 人工 decision evidence；W37 activation implementation gate 只读检查 W35/W36 是否仍满足 separate implementation discussion readiness；W38 adapter routing status 只读声明 controlled adapter disabled 且默认 Send/stream path 均为 legacy stream；W39 contract harness 只读验证 disabled adapter contract；W40 adapter dry run 只显式验证 write-disabled invocation contract；W41 dry-run review 只记录 metadata-safe 人工 decision evidence；W42 implementation readiness 只读组合 W37/W39/W40/W41 证据；W43 controlled preview 先查 W42，blocked 不运行 runtime/不创建 AgentRun，ready 后只运行 write-disabled/zero-tool preview 并写 metadata-safe adapter preview AgentRun audit；W44 controlled preview review 只记录人工 metadata-safe decision evidence，summary 只读；W45 controlled preview approval readiness 只读检查 W42/W44 证据和 approved W43 preview AgentRun 当前安全状态；W46 cutover implementation plan draft 只读基于 W45 readiness 生成人工 planning sections；W47 cutover plan review 只记录 metadata-safe 人工 decision evidence；W48 cutover plan approval readiness 只读检查当前 plan approval/digest/isolation 状态；W49 route guard scaffold 只让默认 Chat 入口 fail-closed 地确认 `legacy_stream`；W50 cutover invocation harness 只允许默认 Chat 入口保持 `legacy_guarded`、write-disabled、zero-tool、side-effect-free；W51 invocation plan 只允许默认 Chat 入口选择 `legacy_stream`，并声明 `controlled_adapter` 仍是 disabled candidate；W52 invocation boundary 只允许默认 Chat 入口进入 `legacy_stream` callsite，并要求 legacy entry 前无 runtime/model/tool/business write 副作用；W53 typed callsite contract 只允许默认 Chat 入口用类型化 send/stream callsite 匹配各自 legacy route path 和 contract shape。它们仍不是默认 Chat migration。
- W54-W57 是 default Chat adapter guard/status 收口：W54 只同步权威路线，W55 只加 ordinary-entry preflight / side-effect lock，W56 只加只读 ordinary-entry preflight status surface，W57 只加只读 narrow implementation discussion gate。普通 Send / `send_message` / `start_stream_message` 仍只能进入 `legacy_stream`，不得调用 W56 status command 或 W57 discussion gate，也不能把 `statusReady` 或 `eligible` 解释为迁移许可。
