...
> Date: 2026-06-04
> Status: W127 Backend Completion Goal 1 complete; default Chat remains `legacy_stream`
> Scope: current LifeModel-governed Agent Runtime program, completed W1-W127 baseline, and the pre-UI backend completion program

## 1. Purpose

This document is the program baseline for the next OpenLife development cycle.
As of W127, the major scoped blocks are complete: W65-W72 default Chat adapter
...
And Schemas. These blocks prove contracts, readiness, proposal-first writes,
trace visibility, non-default product/runtime surfaces, and the first
LifeEvent/Signal schema bridge into EvidenceStore candidate evidence. They do
...
The next Goal-mode implementation entry is Backend Completion Goal 2 from
`plans/lifemodel_governed_backend_completion_goal_spec.md`: Evidence Graph v1
W128-W130. That spec turns the remaining backend blockers into one system-level
pre-UI program: Evidence Graph, Maturation Engine v1, governed
materialization/versioned truth, RuntimeHSPacket guidance integration,
ModelRouter/Privacy and tool governance hardening, backend golden paths, and UI
read model contract freeze. Use it before starting CLI Goal-mode development.
...
6. `plans/lifemodel_governed_runtime_progress.md`
   - Compact W1-W127 status table and compressed guardrail map.
