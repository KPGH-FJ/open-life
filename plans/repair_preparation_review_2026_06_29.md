...
| P1 | Sprint 1 implied it could answer whether data left the machine before provider transmission logging exists. | No `ProviderTransmissionLogEntry` source/test symbol exists; Sprint 5 already says the log is proposed. | Sprint 1 now only exposes external-transmission instrumentation status unless positive runtime evidence exists; definitive sent/not-sent history is deferred to Sprint 5. |
| P1 | Candidate command gates could pass without proving the intended tests ran. | `provider_transmission` has no current source/test symbol; `AgentRunDetail.tsx` exists but no `AgentRunDetail.test.tsx`; cargo/frontend filters may match zero tests if not checked. | Readiness index and sprint RFCs now require exact test names/files and non-zero matched/passed counts; Sprint 2/Sprint 5 call out missing tests explicitly. |
