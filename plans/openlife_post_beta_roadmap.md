...
- [ ] 将 Builder prompt、Calibration prompt（若继续为模型生成）迁入或明确保留为专用 legacy 边界；legacy scheduler generation 已明确保留为 compatibility-only，不允许正式 governed path 调用
