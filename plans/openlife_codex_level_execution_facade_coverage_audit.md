...
Status: Codex-level coverage audit / Skill runtime pre-migration safety net
...
- **Skill runtime**: not migrated. `src-tauri/src/commands/execution.rs::run_skill` now delegates to `run_skill_with_state`, but the real path remains SkillRegistry prompt construction → required stored `AgentSpec` resolution → `AgentRuntime::execute_task_with_spec` for PromptStack/context governance → `InferenceScheduler::generate_governed` for skill JSON generation → skill envelope parsing/validation → Review Center proposal creation. This is a mixed model-generation plus skill-envelope/proposal execution path. It does not call `run_tauri_agent_task`, `handle_agent_loop_fallback`, Scheduled/Replay/Plan wrappers, or Chat fallback. The frontend direct caller is `frontend/src/tauri.ts::runSkill`; `WorkspaceOverview` expects success to return `{ runId, status, summary, generatedProposals }` and currently has no custom failure UI beyond the rejected invoke.
...
- **Skill runtime**: this path remains deliberately outside ExecutionFacade. It mixes model generation, SkillRegistry parsing/validation, `AgentSpec`, PromptStack, `AgentRun`, and ProposalStore semantics. Current safety net proves missing `AgentSpec` fails closed without run/model/fallback; AgentSpec restricted toolsets block skills whose declared `allowed_tools` are not permitted; PromptStack and model-generation failures persist failed Skill runs with readable errors; success keeps the frontend response shape. It must not be claimed as migrated until a Skill-specific facade owns this mixed contract explicitly.
...
- **Skill runtime**: still not migrated. The migration-prep audit now documents the true mixed path and locks no Chat fallback / no wrapper masquerade with `skill_runtime_remains_unmigrated_no_chat_fallback_source_audit`. Remaining prerequisites before migration are: define a Skill-specific facade contract, decide whether skill model generation and envelope parsing are one runtime outcome or two phases, define failure/event semantics for parse warnings vs hard failures, and add deterministic model-output fixtures or an injectable scheduler seam before moving execution.
...
- `skill_runtime_remains_unmigrated_no_chat_fallback_source_audit`
