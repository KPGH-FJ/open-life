...
> 基于 PRD v3.0 与当前代码基线持续更新
> 当前定位：可持续试用 Alpha，正在向 Beta 试用准备收口
> 当前主线：不是继续平铺新功能，而是把试用主链路、解释性和治理能力做完整
...
- **Phase 1：试用稳定化**
  - Safe Mode / 恢复控制台 / 启动降级提示 / 向量索引重建 / 高风险写保护
- **Phase 2：核心产品路径**
  - Onboarding → Settings → Builder → Chat → Dashboard 的首轮路径已明显成型
- **Phase 3：解释性产品化**
  - Builder Review、Calibration、Builder 完成页、Dashboard、Chat 已开始统一“为什么这样建议”
- **Phase 4：治理表达**
  - Memory / Calibration / VersionControl 已开始形成“记住了什么 → 为什么建议变化 → 如何回滚”的治理链
- **Phase 5：Beta 准备（前半段）**
  - App 顶部试用状态、Settings 试用闭环定义、Beta readiness 入口已接上

### 0.2 当前最重要的判断

项目现在不需要推倒重写，但也还不应该继续无节制扩功能。

接下来的重点应该固定为：

1. 继续收口试用主链路
2. 把跨页面说明、治理关系和错误修复路径讲清楚
3. 为后续“边试用边修改”建立稳定基线
...
1. README / 试用说明 / 常见错误路径继续统一
2. 建立更贴近真实桌面试用的 smoke 验证清单
3. 开始一轮面向真实试用的系统性 debug，而不是继续泛化扩功能
