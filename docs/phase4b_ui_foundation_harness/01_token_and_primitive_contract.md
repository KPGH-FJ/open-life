...
| Control boundary | #858585; 3.69:1 on white and 3.30:1 on sunken hover               |
...
Semantic component geometry, such as toggle track dimensions, also lives in
the token file; it is not exposed as a second general spacing scale. Contract
tests reject literal colors and type below 12px in foundation CSS consumers.
...
| Text field       | label plus optional description   | 3:1 boundary and focus ring            | disabled reason required                          | error has `aria-invalid` and linked text                                   |
...
- A disabled OpenLife action, icon, field, or toggle without a visible
  `disabledReason` throws. Loading actions instead expose a visible busy label.
...
- Dynamic harness feedback has one polite live-region owner.
