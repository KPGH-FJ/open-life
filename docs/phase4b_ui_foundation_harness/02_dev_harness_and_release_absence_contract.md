...
The Vite config uses MPA mode plus an HTML-entry boundary. `/`, `/index.html`,
and the previously misreported `/phase4b/` URL return `404` with a pointer to
the canonical path; none may load production `src/main.tsx`.
...
`beforeBuildCommand` is an inline, working-directory-independent hard failure
for release/package use. The rejection was verified from both repository root
and `src-tauri/`.
