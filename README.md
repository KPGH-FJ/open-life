...
当前项目处于 **W21 Reviewed Pilot Response Promotion** 阶段：

- **ReAct 执行闭环已建立**：AgentLoop 迭代执行、Action Parser JSON envelope、Tool Registry 统一注册、Permission/Proposal/Replay 闭合。
- **W1-W21 已完成**：当前已经建立 Runtime Migration Gate 只读诊断层、Settings evidence surface、controlled Chat migration pilot eligibility 只读资格检查、Chat 页面显式单轮 Controlled Pilot，以及 reviewed pilot response promotion；完整状态索引见 [LifeModel-Governed Runtime Progress](/Users/fujing/Desktop/偶来福/plans/lifemodel_governed_runtime_progress.md)。
...
- **下一步仍不能直接替换默认 Chat**：W21 只是 reviewed promotion 台阶。默认 Chat 仍未迁移；默认 `Send` / `send_message` / `start_stream_message` 路径保持现状。
...
### W1-W21: LifeModel-Governed Runtime Preview, Gate Evidence, Controlled Pilot, And Promotion
...
5. Pilot response 默认隔离；只有用户显式点击 `Promote Pilot Response` 并确认 review 后，才写入一条 ordinary assistant message。不得自动 promotion，不得把 promotion 当成默认 Chat 迁移。
