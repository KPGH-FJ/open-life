...
- **Default Chat Adapter Narrow Implementation Plan Draft 已完成**：W58 新增只读 `draft_default_chat_adapter_narrow_implementation_plan`、前端 wrapper 与 Settings 面板。它先调用 W57 discussion gate；blocked 时返回 `draftReady=false`、blocking reasons、空 plan sections 和无 digest；eligible 时只返回 metadata-safe human-review plan sections 与 stable digest。它不创建 Chat/AgentRun/Evidence/Proposal/Memory/LifeModel/MCP audit/external write 记录，不运行 runtime/model/tool/preview，不切换 routing，只是更窄 adapter implementation slice 的人工评审草案，不是 default Chat migration。
- **Default Chat Adapter Narrow Implementation Plan Review Evidence 已完成**：W59 新增 `record_default_chat_adapter_narrow_implementation_plan_review_decision` 与只读 `get_default_chat_adapter_narrow_implementation_plan_review_summary`、前端 wrapper 与 Settings 面板。record command 先调用 W58 draft；blocked draft 的 approve 不写 evidence，ready draft 的 approve/reject/request_rework 只写 metadata-safe Evidence。reviewer note 仅保存 checksum/length/category；不保存 raw note、raw prompt/output、tool payload 或 plan content。Settings 只提供显式记录/刷新，不提供 save/promote/switch/migrate/enable/activate 操作。
- **Default Chat Adapter Narrow Implementation Plan Approval Readiness Gate 已完成**：W60 新增只读 `check_default_chat_adapter_narrow_implementation_plan_approval_readiness`、前端 wrapper 与 Settings 面板。它组合当前 W58 draft 与 W59 latest review evidence，只有 latest approve、当前 narrow plan digest 匹配、W57 仍 eligible、default Chat unchanged、controlled adapter disabled、automatic migration disabled 且 send/stream 均保持 `legacy_stream` 时才 ready。它不创建记录、不运行 runtime/model/tool/preview、不切换 routing；ready 只表示后续 narrow adapter implementation discussion evidence 仍然 current，不是 default Chat migration。
