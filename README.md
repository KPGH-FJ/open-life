...
当前项目处于 **W19 Sustained Gate Evidence / Pilot Eligibility** 阶段：

- **ReAct 执行闭环已建立**：AgentLoop 迭代执行、Action Parser JSON envelope、Tool Registry 统一注册、Permission/Proposal/Replay 闭合。
- **W1-W19 已完成**：当前已经建立 Runtime Migration Gate 只读诊断层、Settings evidence surface，以及 controlled Chat migration pilot eligibility 只读资格检查；完整状态索引见 [LifeModel-Governed Runtime Progress](/Users/fujing/Desktop/偶来福/plans/lifemodel_governed_runtime_progress.md)。
...
- **下一步仍不能直接替换默认 Chat**：W20 才可能进入 very small controlled Chat migration pilot，且必须保留 fallback。即使 W19 eligibility 为 true，也只代表满足最低资格，不代表默认 Chat 可以自动迁移。
...
### W1-W19: LifeModel-Governed Runtime Preview, Gate Evidence, And Pilot Eligibility
...
4. 后续 Chat migration 的最低准入条件：gate evidence 连续干净、fallback 可用、preview outer AgentRun 是主 trace、inner ReAct run id 仅作 child metadata、无真实外部写入、proposal-first 未破坏、`make ci` 通过。
5. W20 才可能进入 very small controlled Chat migration pilot，且必须保留 fallback；eligible 不等于自动替换默认 Chat。
