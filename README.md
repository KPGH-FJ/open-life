...
当前项目处于 **W25 Reviewed Migration Plan Draft Generator** 阶段：

- **ReAct 执行闭环已建立**：AgentLoop 迭代执行、Action Parser JSON envelope、Tool Registry 统一注册、Permission/Proposal/Replay 闭合。
- **W1-W25 已完成**：当前已经建立 Runtime Migration Gate 只读诊断层、Settings evidence surface、controlled Chat migration pilot eligibility 只读资格检查、Chat 页面显式单轮 Controlled Pilot、reviewed pilot response promotion、promotion 后的 source/target session 验证、metadata-safe promotion evidence 记录与只读 summary、基于既有 promotion evidence 的只读 readiness gate，以及 reviewed migration plan draft 只读草案生成器；完整状态索引见 [LifeModel-Governed Runtime Progress](/Users/fujing/Desktop/偶来福/plans/lifemodel_governed_runtime_progress.md)。
...
- **下一步仍不能直接替换默认 Chat**：W25 只是基于既有 readiness 结果的只读人工评审草案。默认 Chat 仍未迁移；默认 `Send` / `send_message` / `start_stream_message` 路径保持现状，且不得调用 eligibility、gate、preview、promotion、evidence recorder、promotion readiness gate 或 migration draft command。
...
| MultiStrategy Runtime | Preview/audit-ready：`run_multi_strategy_agent_preview` 可选择 ReAct/PlanExecute/Blocked payload，并写入 metadata-safe 外层 AgentRun audit；Settings Runtime Migration Gate、Pilot eligibility、Promotion evidence summary、Promotion readiness gate 和 Draft Migration Plan 只读展示 gate/promotion evidence 与人工评审草案；Chat 有 W20 显式 Controlled Pilot 单轮入口、W21 reviewed promotion、W22 source-bound validation、W23 metadata-safe promotion evidence recorder、W24 readiness gate 和 W25 draft plan generator | 继续保持默认 Chat 不迁移；promotion 只是用户确认且 source/target session 一致后写入 assistant message，并记录 metadata-safe evidence 的受控台阶；readiness pass 和 migration draft 都只表示可进入人工讨论，不是迁移许可 |
...
### W1-W25: LifeModel-Governed Runtime Preview, Gate Evidence, Controlled Pilot, Promotion Evidence, Readiness, And Draft Planning
